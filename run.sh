...
# Provena one-click deploy and start script
...
print_banner() {
    ...
    echo ""
    echo "=========================================="
    echo "  Provena Deploy & Start Script"
...
BINARY_NAME="provena"
# This script starts the web console, and the console is an opt-in build: a
# plain `go build` now produces the CLI-only binary. Set GO_BUILD_TAGS="" only if
# you point this script at a different entrypoint.
GO_BUILD_TAGS="${GO_BUILD_TAGS:--tags webconsole}"
...
build_go_project() {
    ...
    (
        set +e  # disable errexit in subshell
        export GOPROXY="$GOPROXY"
        go build $GO_BUILD_TAGS -o "$BINARY_NAME" ./cmd/provena >"$GO_BUILD_LOG" 2>&1
...
build_go_project_quiet() {
    ...
    GO_BUILD_LOG=$(mktemp)
    if ! GOPROXY="$GOPROXY" go build $GO_BUILD_TAGS -o "$BINARY_NAME" ./cmd/provena >"$GO_BUILD_LOG" 2>&1; then
        error "Build failed"
        echo ""
        info "Build error details:"
        cat "$GO_BUILD_LOG" | sed 's/^/  /'
...
need_rebuild() {
    ...
    if [ "$BINARY_NAME" -ot cmd/provena/main.go ] || \
...
main() {
    ...
    if [ "$RESET_ADMIN_PASSWORD" -eq 1 ]; then
        ...
        if [ "${#FORWARD_ARGS[@]}" -gt 0 ]; then
            exec "./$BINARY_NAME" serve -config "$CONFIG_FILE" --reset-admin-password "${FORWARD_ARGS[@]}"
        else
            exec "./$BINARY_NAME" serve -config "$CONFIG_FILE" --reset-admin-password
        fi
    fi
    ...
    if [ "$USE_HTTPS" -eq 1 ]; then
        info "Starting Provena server (HTTPS + HTTP/2, self-signed cert)..."
        note "For plain HTTP, use: $0 --http"
    else
        info "Starting Provena server (HTTP)..."
    fi
    echo "=========================================="
    echo ""
    ...
    if [ "$USE_HTTPS" -eq 1 ]; then
        if [ "${#FORWARD_ARGS[@]}" -gt 0 ]; then
            exec "./$BINARY_NAME" serve -config "$CONFIG_FILE" --https "${FORWARD_ARGS[@]}"
        else
            exec "./$BINARY_NAME" serve -config "$CONFIG_FILE" --https
        fi
    else
        if [ "${#FORWARD_ARGS[@]}" -gt 0 ]; then
            exec "./$BINARY_NAME" serve -config "$CONFIG_FILE" --http "${FORWARD_ARGS[@]}"
        else
            exec "./$BINARY_NAME" serve -config "$CONFIG_FILE" --http
        fi
    fi
}
