...
func (r *Retriever) Search(ctx context.Context, req *SearchRequest) ([]*RetrievalResult, error) {
	...
	if r.config != nil && strings.EqualFold(strings.TrimSpace(r.config.Mode), "lexical") {
		return r.lexicalSearch(req)
	}
...
// lexicalSearch is the offline-safe fallback used by v3-minimal. It scores
// Markdown items by matched query terms and never invokes an embedding API.
func (r *Retriever) lexicalSearch(req *SearchRequest) ([]*RetrievalResult, error) {
	if r == nil || r.db == nil {
	...
	query := `SELECT id, category, title, file_path, COALESCE(content,''), created_at, updated_at FROM knowledge_base_items`
	args := []interface{}{}
	if strings.TrimSpace(req.RiskType) != "" {
		query += " WHERE category = ? COLLATE NOCASE"
		args = append(args, strings.TrimSpace(req.RiskType))
	}
	rows, err := r.db.Query(query, args...)
	if err != nil {
		return nil, fmt.Errorf("本地知识库查询失败: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Category, &item.Title, &item.FilePath, &item.Content, &created, &updated); err != nil {
			return nil, err
		}
		...
		item.CreatedAt = parseKnowledgeTime(created)
		item.UpdatedAt = parseKnowledgeTime(updated)
		if item.UpdatedAt.IsZero() {
			item.UpdatedAt = item.CreatedAt
		}
		candidates = append(candidates, candidate{item: item, score: float64(hits) / float64(len(terms))})
	}
	...
	for _, c := range candidates {
		out = append(out, &RetrievalResult{Item: c.item, Chunk: &KnowledgeChunk{ID: "lexical-" + c.item.ID, ItemID: c.item.ID, ChunkIndex: 0, ChunkText: c.item.Content}, Similarity: c.score, Score: c.score})
	}
	return out, nil
}
