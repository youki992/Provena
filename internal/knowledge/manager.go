...
// SearchLexical provides a deterministic local knowledge search for isolated
// deployments where an embedding endpoint is not available. It reads the same
// Markdown/SQLite source as vector mode and deliberately never calls an LLM,
// reranker, or embedding service.
func (m *Manager) SearchLexical(query, category string, topK int) ([]*RetrievalResult, error) {
	...
	byID := make(map[string]*KnowledgeItem)
	for _, term := range terms {
		if len([]rune(term)) < 2 {
			continue
		}
		summaries, err := m.SearchItemsByKeyword(term, category)
		if err != nil {
			return nil, err
		}
		for _, summary := range summaries {
			if _, ok := byID[summary.ID]; ok {
				continue
			}
			item, getErr := m.GetItem(summary.ID)
			if getErr == nil {
				byID[item.ID] = item
			}
		}
	}
	results := make([]*RetrievalResult, 0, len(byID))
	for _, item := range byID {
		...
		results = append(results, &RetrievalResult{Item: item, Chunk: &KnowledgeChunk{ID: "lexical-" + item.ID, ItemID: item.ID, ChunkIndex: 0, ChunkText: content}, Similarity: score, Score: score})
	}
