...
func TestLegacyQueryExecutionResultAliasesPersistedExecution(t *testing.T) {
	server := NewServer(zap.NewNop())
	RegisterExecutionControlTools(server, nil)

	executionID := server.FinishToolExecution(context.Background(), "", "gau", map[string]interface{}{"domain": "example.test"}, "warning\nhttps://example.test/endpoint", nil)
	if executionID == "" {
		t.Fatal("missing execution id")
	}
	...
	if err != nil {
		t.Fatalf("query_execution_result: %v", err)
	}
	body := ToolResultPlainText(result)
	if result == nil || result.IsError || !strings.Contains(body, "https://example.test/endpoint") {
		t.Fatalf("legacy query result = %+v body=%s", result, body)
	}
}
