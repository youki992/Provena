...
func createSDKClient(ctx context.Context, serverCfg config.ExternalMCPServerConfig, logger *zap.Logger) (ExternalMCPClient, error) {
	...
	case "stdio":
		...
		if strings.TrimSpace(serverCfg.WorkingDir) != "" {
	...
	if err != nil {
		if transport == "stdio" && strings.Contains(err.Error(), `calling "initialize": invalid character`) {
			return nil, fmt.Errorf("连接失败：stdio 进程未返回有效的 MCP JSON-RPC 消息；该程序很可能是普通命令行工具，而不是 MCP Server。请改用 MCP stdio 服务，或为该工具增加 MCP 包装器（原始错误: %w）", err)
		}
