...
func TestExternalMCPManager_ValidateClientRefreshesTools(t *testing.T) {
	manager := NewExternalMCPManager(zap.NewNop())
	t.Cleanup(manager.StopAll)
	manager.configs["lab"] = config.ExternalMCPServerConfig{
		Type:              "http",
		URL:               "http://127.0.0.1:8081/mcp",
		ExternalMCPEnable: true,
	}
	client := &failingExternalMCPClient{}
	manager.clients["lab"] = client

	tools, err := manager.ValidateClient(context.Background(), "lab")
	if err != nil {
		t.Fatalf("ValidateClient returned error: %v", err)
	}
	if len(tools) != 1 || tools[0].Name != "fail_tool" {
		t.Fatalf("tools = %#v, want fresh tools/list result", tools)
	}
	if count, err := manager.GetToolCount("lab"); err != nil || count != 1 {
		t.Fatalf("cached tool count = %d, err=%v; want 1", count, err)
	}
}
