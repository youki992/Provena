...
func (m *ExternalMCPManager) AddOrUpdateConfig(name string, serverCfg config.ExternalMCPServerConfig) error {
	...
	if m.isEnabled(serverCfg) {
		// 统一走 startClient，确保连接失败时会保留 error 状态和具体错误，
		// 管理页面才能区分“尚未连接”和“配置/服务连接失败”。
		go func() {
			if err := m.startClient(name, false); err != nil {
				m.logger.Error("自动连接外部MCP失败", zap.String("name", name), zap.Error(err))
			}
		}()
	}
...
// ValidateClient establishes (or reuses) a server connection and performs a
// fresh tools/list request. It is intentionally synchronous so the management
// UI can report the real handshake/list-tools failure instead of showing a
// misleading "started" state. A newly created client is retained on success,
// which also makes the discovered tools immediately available to agents.
func (m *ExternalMCPManager) ValidateClient(ctx context.Context, name string) ([]Tool, error) {
	name = strings.TrimSpace(name)
	if name == "" {
		return nil, fmt.Errorf("MCP名称不能为空")
	}
	if ctx == nil {
	...
	m.mu.RUnlock()
	if !exists {
		return nil, fmt.Errorf("配置不存在: %s", name)
	}

	// 保存/启动后客户端可能正在后台握手。等待这一次握手结束，避免验证
	// 按钮把正在连接的客户端关闭并重新创建，造成重复进程或状态抖动。
	if client != nil && client.GetStatus() == "connecting" {
		if err := waitForExternalMCPClient(ctx, client); err != nil {
			return nil, err
		}
	}
	...
	if client == nil || !client.IsConnected() {
		...
		if client == nil {
			return nil, fmt.Errorf("无法创建客户端：不支持的传输模式或配置不完整")
		}
		m.setClientStatus(client, "connecting")
		m.mu.Lock()
		m.clients[name] = client
		...
		m.setClientStatus(client, "connected")
		m.onClientConnected(name)
	}

	if !client.IsConnected() {
		return nil, fmt.Errorf("客户端未连接（状态: %s）", client.GetStatus())
	}

	// A manual validation must bypass the normal 60-second cache. Otherwise a
	// stale tool list can make a repaired MCP still appear broken.
	m.toolCacheMu.Lock()
	delete(m.toolCache, name)
	m.toolCacheMu.Unlock()
	tools, err := client.ListTools(ctx)
	if err != nil {
		m.handleConnectionDead(name, client, err)
		m.mu.Lock()
		m.errors[name] = err.Error()
		m.mu.Unlock()
...
func waitForExternalMCPClient(ctx context.Context, client ExternalMCPClient) error {
	if client == nil {
		return fmt.Errorf("MCP客户端不存在")
	}
	...
	for {
		...
		if status != "connecting" {
			// 后续逻辑会关闭失败/断开的旧客户端并重新验证一次。
			return nil
		}
		select {
		case <-ctx.Done():
			return fmt.Errorf("等待 MCP 连接完成失败: %w", ctx.Err())
		case <-ticker.C:
		}
	}
}
...
func (m *ExternalMCPManager) StartAllEnabled() {
	...
	for name := range configs {
		if m.isEnabled(configs[name]) {
			go func(n string) {
				if err := m.startClient(n, false); err != nil {
