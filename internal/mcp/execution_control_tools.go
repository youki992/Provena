...
func RegisterExecutionControlTools(server *Server, external *ExternalMCPManager) {
	...
	// Backward-compatible alias for the older tool manifest. Older Pi runs
	// still emit query_execution_result; route it to the same persisted
	// execution store instead of letting the generic internal-tool handler
	// reject it. The optional page/search/filter fields are honored so legacy
	// prompts can retrieve large results incrementally.
	server.RegisterTool(Tool{
		Name:             builtin.ToolQueryExecutionResult,
		Description:      "兼容旧名称：查询工具执行结果，支持分页、搜索和过滤。",
		ShortDescription: "查询工具执行结果（兼容旧名称）",
		InputSchema: map[string]interface{}{
			"type": "object",
			"properties": map[string]interface{}{
				"execution_id": map[string]interface{}{"type": "string", "description": "工具执行 ID"},
				"page":         map[string]interface{}{"type": "integer", "description": "页码，从 1 开始，默认 1"},
				"limit":        map[string]interface{}{"type": "integer", "description": "每页行数，默认 100，最大 500"},
				"search":       map[string]interface{}{"type": "string", "description": "结果行搜索关键词"},
				"filter":       map[string]interface{}{"type": "string", "description": "结果行过滤关键词"},
			},
			"required": []string{"execution_id"},
		},
	}, func(ctx context.Context, args map[string]interface{}) (*ToolResult, error) {
		...
		return textToolResult(formatExecutionQueryForModel(exec, args), false), nil
	})
}

func formatExecutionQueryForModel(exec *ToolExecution, args map[string]interface{}) string {
	if exec == nil {
		return "execution: null"
	}
	page := intArg(args, "page", 1, 1000000)
	limit := intArg(args, "limit", 100, 500)
	filter := stringArg(args, "filter")
	search := stringArg(args, "search")
	text := ""
	if exec.Result != nil {
		text = ToolResultPlainText(exec.Result)
	}
	...
	payload := map[string]interface{}{
		...
		"result":       strings.Join(selected[start:end], "\n"),
	}
	if exec.Error != "" {
