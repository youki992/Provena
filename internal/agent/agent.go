...
type Tool struct {
	...
	// Local is runtime metadata used by the Pi Harness to distinguish a local
	// CLI tool from a platform-registered or external MCP tool. It is not sent
	// in OpenAI tool definitions.
	Local bool `json:"-"`
}
	...
	if a.externalMCPMgr != nil {
		...
		if err != nil {
			...
			for _, externalTool := range externalTools {
				...
				// tool_enabled 非空时是显式白名单。外部 MCP 经常暴露几十个
				// 工具；把未列出的工具默认带给模型既违背最小权限，也可能让
				// 严格的模型提供方拒绝整个 tool schema 请求。
				enabled := false
				if cfg, exists := externalMCPConfigs[mcpName]; exists {
					enabled = externalMCPToolEnabled(cfg, actualToolName)
				}
...
// externalMCPToolEnabled applies the external-MCP tool policy consistently.
// An omitted/empty tool_enabled keeps the legacy "all tools enabled" behavior.
// Once at least one tool is listed, it is an allowlist: every unlisted tool is
// unavailable to the model. This avoids silently expanding an operator's tool
// surface when an MCP server gains new endpoints.
func externalMCPToolEnabled(cfg config.ExternalMCPServerConfig, toolName string) bool {
	if !cfg.ExternalMCPEnable {
		return false
	}
	if len(cfg.ToolEnabled) == 0 {
		return true
	}
	return cfg.ToolEnabled[toolName]
}
