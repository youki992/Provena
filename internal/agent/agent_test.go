...
func TestToolsForRolePreservesLocalToolMetadataWithoutRoleFilter(t *testing.T) {
	logger := zap.NewNop()
	mcpServer := mcp.NewServer(logger)
	mcpServer.RegisterTool(mcp.Tool{
		Name:        "waybackurls",
		Description: "历史 URL 收集",
		Local:       true,
	...
	ag := NewAgent(&config.OpenAIConfig{}, &config.AgentConfig{}, mcpServer, nil, logger, 1)
	tools := ag.ToolsForRole(nil)
	if len(tools) != 1 || tools[0].Function.Name != "waybackurls" || !tools[0].Local {
		t.Fatalf("local tool metadata was not exposed: %#v", tools)
	}
}

func TestExternalMCPToolEnabledUsesConfiguredToolsAsAllowlist(t *testing.T) {
	cfg := config.ExternalMCPServerConfig{
		ExternalMCPEnable: true,
		ToolEnabled: map[string]bool{
			"arl_submit_scan": true,
			"arl_api_request": false,
		},
	}
	if !externalMCPToolEnabled(cfg, "arl_submit_scan") {
		t.Fatal("configured enabled tool must be available")
	}
	if externalMCPToolEnabled(cfg, "arl_api_request") {
		t.Fatal("configured disabled tool must not be available")
	}
	if externalMCPToolEnabled(cfg, "arl_hidden_tool") {
		t.Fatal("unlisted tool must not bypass the explicit allowlist")
	}
	if !externalMCPToolEnabled(config.ExternalMCPServerConfig{ExternalMCPEnable: true}, "legacy_tool") {
		t.Fatal("an omitted tool_enabled must retain legacy all-tools behavior")
	}
}
