...
func (db *DB) initTables() error {
	...
	CREATE TABLE IF NOT EXISTS captured_packets (
		...
		created_at DATETIME NOT NULL,
		FOREIGN KEY (group_id) REFERENCES packet_groups(id) ON DELETE CASCADE
	);`
	...
	CREATE TABLE IF NOT EXISTS experience_entries (
		...
		source_fgs_path TEXT NOT NULL DEFAULT '',
		source_node_ids TEXT NOT NULL DEFAULT '[]',
		verification_status TEXT NOT NULL DEFAULT 'pending' CHECK (verification_status IN ('pending','verified','human_confirmed','rejected')),
	...
	CREATE INDEX IF NOT EXISTS idx_packet_groups_owner_seen ON packet_groups(owner_user_id, last_seen_at DESC);
	CREATE UNIQUE INDEX IF NOT EXISTS uq_packet_groups_owner_fingerprint ON packet_groups(owner_user_id, fingerprint);
	CREATE INDEX IF NOT EXISTS idx_captured_packets_group_created ON captured_packets(group_id, created_at DESC);
	CREATE INDEX IF NOT EXISTS idx_captured_packets_host_path ON captured_packets(host, path);
	...
	if _, err := db.Exec(createPacketGroupsTable); err != nil {
		return fmt.Errorf("创建packet_groups表失败: %w", err)
	}
	if _, err := db.Exec(createCapturedPacketsTable); err != nil {
		return fmt.Errorf("创建captured_packets表失败: %w", err)
	}
	...
	for tableName, ddl := range map[string]string{
		...
		"experience_entries":           createExperienceEntriesTable,
	} {
	...
	if _, err := db.Exec(`
		CREATE INDEX IF NOT EXISTS idx_experience_status ON experience_entries(verification_status);
		CREATE INDEX IF NOT EXISTS idx_experience_project ON experience_entries(project_id);
		CREATE INDEX IF NOT EXISTS idx_experience_updated ON experience_entries(updated_at);
	`); err != nil {
		return fmt.Errorf("创建experience索引失败: %w", err)
	}
