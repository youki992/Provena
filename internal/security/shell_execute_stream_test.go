...
func TestEinoStreamingShell_SudoFailsFast(t *testing.T) {
	...
	// Match case-insensitively: a localized sudo (Windows prints
	// "已在此计算机上禁用 Sudo") still has to surface its own diagnostic here.
	lower := strings.ToLower(out)
	if !strings.Contains(lower, "sudo") && !strings.Contains(lower, "password") && !strings.Contains(lower, "terminal") {
