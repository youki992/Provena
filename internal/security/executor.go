...
import (
	...
	"github.com/chobits02/provena/internal/tooloutput"
...
// toolNoOutputTimeoutSeconds returns the per-tool override when present.
// Long-running scanners may legitimately produce no output for several minutes;
// ordinary commands keep the global anti-hang guard.
func (e *Executor) toolNoOutputTimeoutSeconds(toolConfig *config.ToolConfig) int {
	if toolConfig != nil && toolConfig.NoOutputTimeoutSeconds != 0 {
		return ResolveShellNoOutputTimeoutSeconds(toolConfig.NoOutputTimeoutSeconds)
	}
	return ResolveShellNoOutputTimeoutSeconds(e.shellNoOutputTimeoutSec)
}
...
func (e *Executor) ExecuteTool(ctx context.Context, toolName string, args map[string]interface{}) (*mcp.ToolResult, error) {
	...
	// 验证命令参数
	if len(cmdArgs) == 0 && !toolConfig.Local {
	...
	// 执行命令。Python 工具在不同系统上可能叫 python、python3 或 py。
	resolvedCommand := ResolveToolCommand(toolConfig.Command)
	resolvedCommand, preparedArgs, cleanupPrepared, prepareErr := PrepareToolCommand(resolvedCommand, cmdArgs)
	if prepareErr != nil {
		return &mcp.ToolResult{Content: []mcp.Content{{Type: "text", Text: prepareErr.Error()}}, IsError: true}, nil
	}
	defer cleanupPrepared()
	cmd := exec.CommandContext(ctx, resolvedCommand, preparedArgs...)
	...
	e.logger.Debug("执行安全工具",
		...
		zap.String("resolvedCommand", resolvedCommand),
	...
	if cb, ok := ctx.Value(ToolOutputCallbackCtxKey).(ToolOutputCallback); (ok && cb != nil) || mcp.MCPExecutionIDFromContext(ctx) != "" {
		...
		output, err = streamCommandOutput(ctx, cmd, cb, e.toolNoOutputTimeoutSeconds(toolConfig), e.toolOutputMaxBytes, spill)
		if err != nil && shouldRetryWithPTY(output) {
			e.logger.Info("检测到工具需要 TTY，使用 PTY 重试",
				zap.String("tool", toolName),
			)
			cmd2 := exec.CommandContext(ctx, resolvedCommand, preparedArgs...)
...
func (e *Executor) executeSystemCommand(ctx context.Context, args map[string]interface{}) (*mcp.ToolResult, error) {
	...
	resolvedShell, shellErr := resolveAgentShell(shell)
	if shellErr != nil {
		return &mcp.ToolResult{Content: []mcp.Content{{Type: "text", Text: shellErr.Error()}}, IsError: true}, nil
	}
	shell = resolvedShell
