...
func TestMCPInvocationPermissionIsSeparateFromMCPAdministration(t *testing.T) {
	...
	if got := permissionForRequest(http.MethodPost, "/api/config/tools/nmap/test"); got != "mcp:execute" {
		t.Fatalf("builtin tool test permission = %q, want mcp:execute", got)
	}
...
func TestPacketCaptureRoutesUseConfigPermissions(t *testing.T) {
	cases := []struct {
		method, path, want string
	}{
		{http.MethodGet, "/api/packet-capture/config", "config:read"},
		{http.MethodPut, "/api/packet-capture/config", "config:write"},
		{http.MethodGet, "/api/packet-capture/ca/download", "config:read"},
		{http.MethodPost, "/api/packet-capture/ca/install-windows", "config:write"},
		{http.MethodGet, "/api/packet-groups", "config:read"},
		{http.MethodGet, "/api/packet-groups/group-1/packets", "config:read"},
		{http.MethodGet, "/api/captured-packets/packet-1", "config:read"},
	}
