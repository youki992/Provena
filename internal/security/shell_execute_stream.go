...
// PrepareCommandForProcessTree configures an arbitrary command to run in an
// isolated process group/session so cancellation can terminate descendants as
// well as the direct child. This is used by integrations such as Pi RPC that
// may launch their own shells and scanners.
func PrepareCommandForProcessTree(cmd *exec.Cmd) {
	if cmd == nil {
		return
	}
	_ = prepareShellCmdSession(cmd)
...
// TerminateCommandTree terminates a command and its descendants/process group.
...
func runShellInBackground(ctx context.Context, command string, w *schema.StreamWriter[*filesystem.ExecuteResponse]) {
	...
	cmd, err := newAgentShellCommand(ctx, command)
	if err != nil {
		_ = w.Send(nil, err)
		return
	}
