...
func permissionForRequest(method, fullPath string) string {
	...
	case strings.HasPrefix(path, "/robot"):
		return "robot:write"
	case strings.HasPrefix(path, "/pi-agent"):
		return "agent:local-execute"
	...
	case strings.HasPrefix(path, "/packet-capture"):
		// The proxy, CA and capture settings are local platform configuration.
		// Reads are available with the same permission as the settings page;
		// starting, configuring or installing the CA requires config:write.
		return crudPermission(method, "config")
	case strings.HasPrefix(path, "/packet-groups"), strings.HasPrefix(path, "/captured-packets"):
		return "config:read"
	case strings.HasPrefix(path, "/config/tools/") && method == http.MethodPost && strings.HasSuffix(path, "/test"):
		// Builtin tool tests execute the real MCP handler and therefore require
		// the same permission as a normal MCP invocation, not config:write.
		return "mcp:execute"
	case strings.HasPrefix(path, "/config"):
		return crudPermission(method, "config")
	case strings.HasPrefix(path, "/terminal"):
		return "terminal:execute"
	case strings.HasPrefix(path, "/local-tools"):
		if method == http.MethodGet || method == http.MethodHead {
			return "config:read"
		}
		return "config:write"
	...
	case path == "/mcp":
		return "mcp:execute"
	case strings.HasPrefix(path, "/external-mcp") && method == http.MethodPost && strings.HasSuffix(path, "/test"):
		// The management page may validate connectivity with mcp:write, but
		// invoking a real external tool must also pass the dedicated execution
		// permission enforced by ExternalMCPManager.
		return "mcp:external:execute"
