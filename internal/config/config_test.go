...
func TestPiActivityTimeoutEffectiveIsBounded(t *testing.T) {
	if got := (PiAgentConfig{}).ActivityTimeoutSecondsEffective(); got != 180 {
		t.Fatalf("default Pi activity timeout = %d, want 180", got)
	}
	if got := (PiAgentConfig{ActivityTimeoutSeconds: 30}).ActivityTimeoutSecondsEffective(); got != 30 {
		t.Fatalf("custom Pi activity timeout = %d, want 30", got)
	}
	if got := (PiAgentConfig{ActivityTimeoutSeconds: 9999}).ActivityTimeoutSecondsEffective(); got != 900 {
		t.Fatalf("capped Pi activity timeout = %d, want 900", got)
	}
}
