...
type Config struct {
	...
	PiAgent                PiAgentConfig         `yaml:"pi_agent,omitempty" json:"pi_agent,omitempty"`
	...
	C2                     C2Config              `yaml:"c2,omitempty" json:"c2,omitempty"`                                               // 内置 C2 总开关；未配置时默认关闭（opt-in）
	WebShell               WebShellConfig        `yaml:"webshell,omitempty" json:"webshell,omitempty"`                                   // WebShell 管理能力开关；未配置时默认关闭（opt-in）
	PacketCapture          PacketCaptureConfig   `yaml:"packet_capture,omitempty" json:"packet_capture,omitempty"`                       // 本地抓包代理与 HTTPS 明文解密
	...
	GlobalSystemPromptPath string                `yaml:"global_system_prompt_path,omitempty" json:"global_system_prompt_path,omitempty"` // 全局系统提示词文件（相对 config.yaml 所在目录）
...
// The proxy is intentionally bound to loopback by default; captured bodies are
// stored as evidence and may be sent verbatim to Pi when explicitly selected.
type PacketCaptureConfig struct {
	...
	SendFullToPi  bool     `yaml:"send_full_to_pi" json:"send_full_to_pi"`
	AllowedCIDRs  []string `yaml:"allowed_cidrs,omitempty" json:"allowed_cidrs,omitempty"`
}

func (c PacketCaptureConfig) HostEffective() string {
...
func (c PacketCaptureConfig) PortEffective() int {
	if c.Port <= 0 {
		return 9081
	}
	return c.Port
...
func (c PacketCaptureConfig) MaxBodyBytesEffective() int {
	if c.MaxBodyBytes <= 0 {
		return 32 * 1024 * 1024
	}
	if c.MaxBodyBytes > 32*1024*1024 {
		return 32 * 1024 * 1024
	}
...
type MultiAgentEinoMiddlewareConfig struct {
	// MaxToolCalls limits actual tool executions per agent run. 0 means unlimited;
	// a negative value is kept as a compatibility alias for unlimited.
	MaxToolCalls int `yaml:"max_tool_calls,omitempty" json:"max_tool_calls,omitempty"`
	// MaxToolCallsPerTool limits repeated use of one tool in a run. 0 means unlimited;
	// a negative value is kept as a compatibility alias for unlimited.
	MaxToolCallsPerTool int `yaml:"max_tool_calls_per_tool,omitempty" json:"max_tool_calls_per_tool,omitempty"`
	// MaxDuplicateToolCalls is the number of times an identical tool+arguments pair may execute.
	// 0 means unlimited; a negative value is kept as a compatibility alias for unlimited.
	MaxDuplicateToolCalls int `yaml:"max_duplicate_tool_calls,omitempty" json:"max_duplicate_tool_calls,omitempty"`
	// MaxToolSearchCalls limits repeated tool-search discovery loops. 0 means unlimited;
	// a negative value is kept as a compatibility alias for unlimited.
	MaxToolSearchCalls int `yaml:"max_tool_search_calls,omitempty" json:"max_tool_search_calls,omitempty"`
...
func (c MultiAgentEinoMiddlewareConfig) MaxToolCallsEffective() int {
	if c.MaxToolCalls > 0 {
		return c.MaxToolCalls
	}
	return 0 // 0 is the middleware's unlimited sentinel.
}

func (c MultiAgentEinoMiddlewareConfig) MaxToolCallsPerToolEffective() int {
	if c.MaxToolCallsPerTool < 0 {
		return 0
	}
	if c.MaxToolCallsPerTool > 0 {
		return c.MaxToolCallsPerTool
	}
	return 0
}

func (c MultiAgentEinoMiddlewareConfig) MaxDuplicateToolCallsEffective() int {
	if c.MaxDuplicateToolCalls < 0 {
		return 0
	}
	if c.MaxDuplicateToolCalls > 0 {
		return c.MaxDuplicateToolCalls
	}
	return 0
}

func (c MultiAgentEinoMiddlewareConfig) MaxToolSearchCallsEffective() int {
	if c.MaxToolSearchCalls < 0 {
		return 0
	}
	if c.MaxToolSearchCalls > 0 {
		return c.MaxToolSearchCalls
	}
	return 0
}
...
type MultiAgentEinoSkillsConfig struct {
	...
	// FilesystemReadOnly disables write_file/edit_file and shell execution while
	// keeping ls/read_file/glob/grep available for evidence inspection.
...
func (c MultiAgentEinoSkillsConfig) EinoSkillFilesystemReadOnlyEffective() bool {
	return c.FilesystemReadOnly != nil && *c.FilesystemReadOnly
}
...
type OpenAIConfig struct {
	...
	// GlobalSystemPrompt is loaded from Config.GlobalSystemPromptPath at runtime.
	// It is excluded from YAML/JSON so prompt text is not exposed by config APIs
	// or written back to config.yaml.
	GlobalSystemPrompt string `yaml:"-" json:"-"`
}

// PiAgentConfig configures the Pi Coding Agent RPC process used by the
// pi_single chat mode. Pi's provider/model catalog remains managed by Pi;
// these fields only select the command and runtime overrides.
type PiAgentConfig struct {
	Enabled bool   `yaml:"enabled" json:"enabled"`
	Command string `yaml:"command,omitempty" json:"command,omitempty"`
	// HarnessMode selects the model-facing runtime. "fgs" is the generic
	// fact/intent graph harness; "legacy" keeps the pre-FGS Pi loop available
	// for compatibility while migrations are in progress.
	HarnessMode          string `yaml:"harness_mode,omitempty" json:"harness_mode,omitempty"`
	HarnessMaxActivities int    `yaml:"harness_max_activities,omitempty" json:"harness_max_activities,omitempty"`
	HarnessWorkerCount   int    `yaml:"harness_worker_count,omitempty" json:"harness_worker_count,omitempty"`
	// ActivityTimeoutSeconds bounds one stateless Decide/Execute Pi RPC
	// activity. The overall task may run for much longer because ARL scans can
	// be asynchronous, but a single model turn must never hold the task open
	// indefinitely.
	ActivityTimeoutSeconds int                `yaml:"activity_timeout_seconds,omitempty" json:"activity_timeout_seconds,omitempty"`
	...
	NoContextFiles         bool               `yaml:"no_context_files" json:"no_context_files"`
	NoSkills               bool               `yaml:"no_skills" json:"no_skills"`
	NoPromptTemplates      bool               `yaml:"no_prompt_templates" json:"no_prompt_templates"`
	NoExtensions           bool               `yaml:"no_extensions" json:"no_extensions"`
	Supervisor             PiSupervisorConfig `yaml:"supervisor,omitempty" json:"supervisor,omitempty"`
}

// PiSupervisorConfig controls the lightweight Pi process that reviews the
// main Pi agent's progress between incomplete rounds. It is deliberately
// tool-less and receives bounded, untrusted observations only.
type PiSupervisorConfig struct {
	Enabled        bool   `yaml:"enabled" json:"enabled"`
	Command        string `yaml:"command,omitempty" json:"command,omitempty"`
	Provider       string `yaml:"provider,omitempty" json:"provider,omitempty"`
	Model          string `yaml:"model,omitempty" json:"model,omitempty"`
	Thinking       string `yaml:"thinking,omitempty" json:"thinking,omitempty"`
	IntervalRounds int    `yaml:"interval_rounds,omitempty" json:"interval_rounds,omitempty"`
	TimeoutSeconds int    `yaml:"timeout_seconds,omitempty" json:"timeout_seconds,omitempty"`
	Prompt         string `yaml:"prompt,omitempty" json:"prompt,omitempty"`
}

func (c PiSupervisorConfig) IntervalRoundsEffective() int {
...
func (c PiSupervisorConfig) TimeoutSecondsEffective() int {
...
func (c PiAgentConfig) CommandEffective() string {
...
func (c PiAgentConfig) HarnessModeEffective() string {
	mode := strings.ToLower(strings.TrimSpace(c.HarnessMode))
	if mode == "legacy" {
		return mode
	}
	return "fgs"
...
func (c PiAgentConfig) HarnessMaxActivitiesEffective() int {
	if c.HarnessMaxActivities <= 0 {
		return 64
	}
	if c.HarnessMaxActivities > 256 {
		return 256
	}
	return c.HarnessMaxActivities
}

func (c PiAgentConfig) HarnessWorkerCountEffective() int {
	if c.HarnessWorkerCount <= 0 {
		return 3
	}
	if c.HarnessWorkerCount > 16 {
		return 16
	}
	return c.HarnessWorkerCount
}

func (c PiAgentConfig) ActivityTimeoutSecondsEffective() int {
	if c.ActivityTimeoutSeconds <= 0 {
		return 180
	}
	if c.ActivityTimeoutSeconds > 900 {
		return 900
	}
	return c.ActivityTimeoutSeconds
}
...
func (c AIChannelConfig) ToOpenAIConfig() OpenAIConfig {
	...
	return OpenAIConfig{
		...
		BaseURL:             expandEnvVar(c.BaseURL),
		Model:               c.Model,
...
func (c *Config) ResolveAIChannel(channelID string) (OpenAIConfig, string, bool) {
	...
	if oa, id, ok := c.AI.ResolveChannel(channelID); ok {
		oa.GlobalSystemPrompt = c.OpenAI.GlobalSystemPrompt
		return oa, id, true
	}
	fallback := c.OpenAI
	fallback.APIKey = expandEnvVar(fallback.APIKey)
	fallback.BaseURL = expandEnvVar(fallback.BaseURL)
	return fallback, NormalizeAIChannelID(channelID), strings.TrimSpace(c.OpenAI.Model) != "" || strings.TrimSpace(c.OpenAI.BaseURL) != ""
}
...
func (c *Config) ApplyDefaultAIChannel() {
	...
	globalSystemPrompt := c.OpenAI.GlobalSystemPrompt
	c.AI.EnsureDefaultFromOpenAI(c.OpenAI)
	if oa, _, ok := c.AI.ResolveChannel(c.AI.DefaultChannel); ok {
		c.OpenAI = oa
		c.OpenAI.GlobalSystemPrompt = globalSystemPrompt
	}
}
...
type AgentConfig struct {
	...
	// WorkspaceRootDir 会话工作目录根路径（curl/wget 下载、read_file/glob/grep 本地分析）；Pi 始终在其下按 conversations/{id} 隔离，避免同项目目标互串。
...
const hitlAuditAgentPromptBase = `你是 Provena 人机协同审计 Agent。审查 Agent 即将执行的工具调用是否会对系统造成实质性损害。
...
type ExternalMCPServerConfig struct {
	...
	// 兼容旧版配置使用的 transport 字段；新配置统一写入 type。
	...
	// WorkingDir sets the child process working directory for stdio MCP servers.
	WorkingDir string `yaml:"working_dir,omitempty" json:"working_dir,omitempty"`
...
type ToolConfig struct {
	...
	Local            bool              `yaml:"local,omitempty"`              // 由本地工具管理模块创建的普通 CLI 工具
	// NoOutputTimeoutSeconds overrides agent.shell_no_output_timeout_seconds for this tool.
	// 0 inherits the global value; -1 disables the no-output idle guard for long-running scanners.
	NoOutputTimeoutSeconds int `yaml:"no_output_timeout_seconds,omitempty"`
}
...
func Load(path string) (*Config, error) {
	...
	// Keep Pi as the default single-agent backend for existing config files
	// that predate the pi_agent section. An explicit pi_agent.enabled: false
	// remains possible once the section is present.
	var rootFields map[string]yaml.Node
	_ = yaml.Unmarshal(data, &rootFields)
	_, hasPiAgentSection := rootFields["pi_agent"]
	hasPiSupervisorSection := false
	if piNode, ok := rootFields["pi_agent"]; ok && piNode.Kind == yaml.MappingNode {
		for i := 0; i+1 < len(piNode.Content); i += 2 {
			if piNode.Content[i].Value == "supervisor" {
				hasPiSupervisorSection = true
				break
			}
		}
	}
	if !hasPiAgentSection && !cfg.PiAgent.Enabled && strings.TrimSpace(cfg.PiAgent.Command) == "" &&
		strings.TrimSpace(cfg.PiAgent.Provider) == "" && strings.TrimSpace(cfg.PiAgent.Model) == "" {
		cfg.PiAgent.Enabled = true
		cfg.PiAgent.Command = "pi"
		cfg.PiAgent.NoSession = true
	}
	// Existing configurations predate the supervisor block. Enable the
	// guardrail by default, while preserving an explicit enabled: false.
	if !hasPiSupervisorSection {
		cfg.PiAgent.Supervisor.Enabled = true
	}
	...
	if cfg.ExternalMCP.Servers != nil {
		for name, serverCfg := range cfg.ExternalMCP.Servers {
			...
			NormalizeExternalMCPServerCommand(&serverCfg)
...
func PrintMCPConfigJSON(mcp MCPConfig) {
	...
	fmt.Println("[Provena] MCP 配置（可复制到 Cursor / Claude Code 使用）：")
...
type C2Config struct {
	// Enabled 为 nil 表示未写配置，按 false 处理。C2 只面向自有/已授权环境，
	// 因此在通用开源默认配置下必须显式开启才会初始化监听器与 C2 MCP 工具。
	Enabled *bool `yaml:"enabled,omitempty" json:"enabled,omitempty"`
}

// EnabledEffective 返回是否启用 C2；未显式配置时默认关闭（opt-in）。
...
// WebShellConfig 控制 WebShell 管理能力是否注册为 MCP 工具。
// 与 C2 一致：这是面向自有/已授权环境的高风险能力，默认关闭，需显式开启。
type WebShellConfig struct {
	Enabled *bool `yaml:"enabled,omitempty" json:"enabled,omitempty"`
}

// EnabledEffective 返回是否启用 WebShell 管理工具；未显式配置时默认关闭（opt-in）。
func (w WebShellConfig) EnabledEffective() bool {
	if w.Enabled == nil {
		return false
	}
	return *w.Enabled
...
type RetrievalConfig struct {
	// Mode selects the retrieval implementation. lexical is fully local
	// SQLite/Markdown keyword retrieval and needs no embedding model; vector
	// preserves the existing embedding + rerank pipeline.
	Mode                string  `yaml:"mode,omitempty" json:"mode,omitempty"`
...
func (c RetrievalConfig) ModeEffective() string {
	if strings.EqualFold(strings.TrimSpace(c.Mode), "lexical") {
		return "lexical"
	}
	return "vector"
}
