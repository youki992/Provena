...
func TestGenerateDraftFromNaturalLanguageHighRiskAddsHITLAndValidGraph(t *testing.T) {
	...
	var hasDynamicAgent, hasHITL, hasConfirmation bool
	for _, node := range result.Graph.Nodes {
		if node.Type == "agent" && strings.Contains(cfgString(node.Config, "instruction"), "不要按预设场景套固定工具链") {
			hasDynamicAgent = true
		}
		if node.Type == "hitl" {
			hasHITL = true
		}
	...
	if !hasDynamicAgent || !hasHITL || !hasConfirmation {
		t.Fatalf("expected dynamic agent, HITL, and confirmation marker; agent=%v hitl=%v confirmation=%v", hasDynamicAgent, hasHITL, hasConfirmation)
	}
}

func TestGenerateDraftFromNaturalLanguagePentestPresetBuildsClosedLoop(t *testing.T) {
	result, err := GenerateDraftFromNaturalLanguage(context.Background(), DraftRequest{
		Prompt: "对 https://target.example.com 做完整渗透测试并输出报告",
		Target: "https://target.example.com",
		Options: DraftOptions{
			IncludeObjective: true,
			AllowHighRisk:    false,
		},
		AvailableTools: []DraftTool{
			{Key: "subfinder", Name: "subfinder", Enabled: true},
			{Key: "nmap", Name: "nmap", Enabled: true},
			{Key: "ffuf", Name: "ffuf", Enabled: true},
			{Key: "nuclei", Name: "nuclei", Enabled: true},
		},
	})
	if err != nil {
		t.Fatalf("GenerateDraftFromNaturalLanguage: %v", err)
	...
	if cfgString(result.Graph.Config, "workflow_mode") != "pentest" {
		t.Fatalf("workflow_mode = %q, want pentest", cfgString(result.Graph.Config, "workflow_mode"))
	}
	...
	for _, node := range result.Graph.Nodes {
		if _, ok := wantLabels[node.Label]; ok {
			wantLabels[node.Label] = true
		}
	}
	for label, seen := range wantLabels {
		if !seen {
			t.Fatalf("expected node label %q in pentest draft", label)
		}
	}
	if !strings.Contains(result.Audit.Assumptions[0], "动态选择") {
		t.Fatalf("expected purpose-driven tool selection assumption: %#v", result.Audit.Assumptions)
	}
	if !result.Audit.NeedsHITL {
		t.Fatalf("expected pentest draft to include HITL gate: %#v", result.Audit)
	}
}
