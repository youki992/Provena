...
func mcpToolAuthorizer(db *database.DB) func(context.Context, string, map[string]interface{}) error {
	return func(ctx context.Context, toolName string, args map[string]interface{}) error {
		...
		case builtin.ToolSearchSpaceAssets:
			// The web information-collection page already uses this permission;
			// keep the Pi/MCP path under the same RBAC capability.
			return require("fofa:execute")
		...
		case builtin.ToolGetToolExecution, builtin.ToolWaitToolExecution, builtin.ToolQueryExecutionResult:
