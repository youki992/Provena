...
// WorkspaceRootDir returns the legacy project/conversation workspace root.
// New agent runs should use ConversationWorkspaceRootDir so downloaded
// evidence cannot leak between targets in the same project.
...
// ConversationWorkspaceRootDir returns an isolated workspace for one
// conversation. Project intelligence is shared through the database; local
// files and command output are deliberately conversation-scoped.
func ConversationWorkspaceRootDir(configuredBase, conversationID string) string {
	base := strings.TrimSpace(configuredBase)
	if base == "" {
		base = filepath.Join("tmp", "workspace")
	}
