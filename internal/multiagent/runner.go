...
import (
	...
	"github.com/chobits02/provena/internal/einomcp"
	"github.com/chobits02/provena/internal/openai"
	"github.com/chobits02/provena/internal/project"
	"github.com/chobits02/provena/internal/reasoning"
...
func RunDeepAgent(
	...
	systemPromptExtra = appendDirectSkillPrompt(systemPromptExtra, appCfg.SkillsDir, userMessage)
	...
	if orchMode != "plan_execute" {
		...
		for _, sub := range effectiveSubs {
			...
			if instr == "" {
				instr = "你是 Provena 中的专业子代理，在授权渗透测试场景下协助完成用户委托的子任务。优先使用可用工具获取证据，回答简洁专业。"
			}
			...
			subToolBudget := newToolCallBudget(&ma.EinoMiddleware)
			...
			if einoSkillMW != nil {
				if einoFSTools && einoLoc != nil {
					subFs, fsErr := subAgentFilesystemMiddleware(ctx, einoLoc, toolInvokeNotify, id, einoExecBegin, einoExecAppendPartial, einoExecRegisterCancel, einoExecUnregisterCancel, einoExecFinish, agentToolTimeoutMinutes(appCfg), agentToolWaitTimeoutSeconds(appCfg), agentShellNoOutputTimeoutSeconds(appCfg), ma.EinoSkills.EinoSkillFilesystemReadOnlyEffective(), nil)
					if fsErr != nil {
				...
				ToolsConfig: adk.ToolsConfig{
					ToolsNodeConfig: compose.ToolsNodeConfig{
						...
						ToolCallMiddlewares: []compose.ToolMiddleware{
							...
							toolCallBudgetMiddleware(subToolBudget),
						},
					},
	...
	orchestratorName := "provena-deep"
	orchDescription := "根据工具用途、参数 schema 和实时证据自主选择 MCP 工具，协调授权安全任务并整理事实与漏洞结果。"
	...
	orchInstruction = strings.TrimSpace(orchInstruction + "\n\n你是动态工具编排者：不要把固定场景映射到固定工具。根据工具用途说明、参数 schema、授权范围和前一步结果自行决定调用、顺序、重复或停止；没有新证据时停止，并把结果沉淀到事实黑板或漏洞结果中。")
	...
	readOnlyFilesystem := ma.EinoSkills.EinoSkillFilesystemReadOnlyEffective()
	if einoLoc != nil && einoFSTools && !readOnlyFilesystem {
		deepBackend = einoLoc
	...
	if einoLoc != nil && einoFSTools && readOnlyFilesystem {
		roFs, fsErr := subAgentFilesystemMiddleware(ctx, einoLoc, toolInvokeNotify, orchestratorName, einoExecBegin, einoExecAppendPartial, einoExecRegisterCancel, einoExecUnregisterCancel, einoExecFinish, agentToolTimeoutMinutes(appCfg), agentToolWaitTimeoutSeconds(appCfg), agentShellNoOutputTimeoutSeconds(appCfg), true, nil)
		if fsErr != nil {
			return nil, fmt.Errorf("orchestrator filesystem 中间件: %w", fsErr)
		}
		if roFs != nil {
			deepHandlers = append(deepHandlers, roFs)
		}
	}
	...
	toolBudget := newToolCallBudget(&ma.EinoMiddleware)
	mainToolsCfg := adk.ToolsConfig{
		ToolsNodeConfig: compose.ToolsNodeConfig{
			...
			ToolCallMiddlewares: []compose.ToolMiddleware{
				...
				hitlToolCallMiddleware(),
				softRecoveryToolMiddleware(),
				toolCallBudgetMiddleware(toolBudget),
			},
		},
		EmitInternalEvents: true,
	}
