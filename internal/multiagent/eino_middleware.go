...
func mergeAlwaysVisibleToolNames(configured []string) []string {
	// Keep only control/evidence tools in the first model context. The previous
	// implementation appended every builtin tool here, which made tool_search
	// effectively a no-op for the exact tool pool we wanted to reduce.
	defaults := []string{
		"exit", "skill", "tool_search", "read_file", "glob", "grep",
		"query_execution_result", "record_vulnerability", "list_vulnerabilities", "get_vulnerability",
	}
