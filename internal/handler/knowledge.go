...
// SetRetrievalMode lets v3-minimal expose a truthful status when it uses
// local lexical search instead of a remote embedding index.
func (h *KnowledgeHandler) SetRetrievalMode(mode string) {
	if h != nil {
		h.retrievalMode = strings.ToLower(strings.TrimSpace(mode))
	}
}
...
func (h *KnowledgeHandler) StartIndex(c *gin.Context) {
	if h.indexer == nil {
		c.JSON(http.StatusOK, gin.H{"message": "当前为本地词法检索模式，无需构建向量索引", "mode": "lexical"})
		return
	}
...
func (h *KnowledgeHandler) ScanKnowledgeBase(c *gin.Context) {
	if h.indexer == nil {
		...
		c.JSON(http.StatusOK, gin.H{"message": "扫描完成；当前为本地词法检索模式，不需要 embedding 索引", "items_to_index": len(itemsToIndex), "mode": "lexical"})
		return
	}
	itemsToIndex, err := h.manager.ScanKnowledgeBase()
	if err != nil {
...
func (h *KnowledgeHandler) GetIndexStatus(c *gin.Context) {
	...
	if h.retrievalMode == "lexical" {
		status["mode"] = "lexical"
		status["indexed_items"] = status["total_items"]
		status["progress_percent"] = 100.0
		status["is_complete"] = true
		status["message"] = "本地词法检索已就绪；不需要 embedding 向量索引"
