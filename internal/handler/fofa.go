...
import (
	...
	openaiClient "github.com/chobits02/provena/internal/openai"
...
// SpaceSearchRequest is the provider-neutral request used by both the HTTP
// page and the Pi/MCP bridge. Credentials stay in the server config; callers
// only provide a query and presentation options.
...
type fofaSearchRequest = SpaceSearchRequest
...
// SpaceSearchResponse is the normalized result shared by all supported
// network-space providers.
...
type fofaSearchResponse = SpaceSearchResponse

// SpaceSearchError carries a safe user-facing error and, when applicable, the
// configuration fields that must be filled before the request can run.
type SpaceSearchError struct {
	Status  int
	Message string
	Need    []string
	EnvKey  []string
...
func (e *SpaceSearchError) Error() string {
	if e == nil {
		return "空间测绘搜索失败"
	}
	return e.Message
...
func (h *FofaHandler) resolveFOFAEmail() string {
	if email := strings.TrimSpace(os.Getenv("FOFA_EMAIL")); email != "" {
		return email
	}
	if h != nil && h.cfg != nil {
		return strings.TrimSpace(h.cfg.FOFA.Email)
	}
	return ""
}

func (h *FofaHandler) resolveAutoSpaceSearchProvider() string {
	if h.resolveAPIKey("fofa") != "" && h.resolveFOFAEmail() != "" {
		return "fofa"
	}
	for _, provider := range []string{"zoomeye", "quake", "shodan"} {
		if h.resolveAPIKey(provider) != "" {
			return provider
		}
	}
	return ""
...
func (h *FofaHandler) ParseNaturalLanguage(c *gin.Context) {
	...
	provider := normalizeSpaceSearchProvider(requestedProvider)
	if requestedProvider == "auto" {
		provider = h.resolveAutoSpaceSearchProvider()
	}
	if provider == "" {
		message := "provider 不支持，可选：auto、fofa、zoomeye、quake、shodan"
		if requestedProvider == "auto" {
			message = "未配置可用的网络空间搜索 API：请至少配置 FOFA（邮箱+API Key）、ZoomEye、Quake 或 Shodan 之一"
			c.JSON(http.StatusBadRequest, gin.H{
				"error": message,
				"need":  []string{"fofa.email + fofa.api_key 或其他 provider.api_key"},
			})
			return
		}
...
// Search is the HTTP adapter for the provider-neutral search service. Keeping
// the actual provider calls below this boundary lets Pi invoke exactly the
// same path as the web page without copying request logic.
func (h *FofaHandler) Search(c *gin.Context) {
	var req SpaceSearchRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "无效的请求参数: " + err.Error()})
		return
	}
	result, err := h.SearchSpaceAssets(c.Request.Context(), req)
	if err != nil {
		writeSpaceSearchError(c, err)
		return
	}
	c.JSON(http.StatusOK, result)
}

// SearchSpaceAssets executes one authorized network-space provider query.
// API credentials are resolved server-side and are never accepted as tool
// arguments, so Pi can search after the administrator configures a provider
// without receiving the provider secret.
func (h *FofaHandler) SearchSpaceAssets(ctx context.Context, req SpaceSearchRequest) (SpaceSearchResponse, error) {
	if ctx == nil {
	...
	if requestedProvider == "auto" {
		provider = h.resolveAutoSpaceSearchProvider()
		if provider == "" {
			return SpaceSearchResponse{}, &SpaceSearchError{
	...
	if provider == "" {
		return SpaceSearchResponse{}, &SpaceSearchError{Status: http.StatusBadRequest, Message: "provider 不支持，可选：auto、fofa、zoomeye、quake、shodan"}
	}
	req.Provider = provider
	req.Query = strings.TrimSpace(req.Query)
	if req.Query == "" {
		return SpaceSearchResponse{}, &SpaceSearchError{Status: http.StatusBadRequest, Message: "query 不能为空"}
	}
	if req.Size <= 0 {
		req.Size = 100
	}
	...
	if provider != "fofa" {
		return h.searchExternalProvider(ctx, provider, req)
	}

	apiKey := h.resolveAPIKey(provider)
	email := h.resolveFOFAEmail()
	if apiKey == "" || email == "" {
		return SpaceSearchResponse{}, &SpaceSearchError{
			Status:  http.StatusBadRequest,
			Message: "FOFA 未配置：需要同时填写账户邮箱和 API Key，或设置环境变量 FOFA_EMAIL 与 FOFA_API_KEY",
			Need:    []string{"fofa.email", "fofa.api_key"},
			EnvKey:  []string{"FOFA_EMAIL", "FOFA_API_KEY"},
		}
	}

	u, err := url.Parse(h.resolveBaseURL(provider))
	if err != nil {
		return SpaceSearchResponse{}, &SpaceSearchError{Status: http.StatusInternalServerError, Message: "FOFA base_url 无效: " + err.Error()}
	}
	params := u.Query()
	params.Set("email", email)
	params.Set("key", apiKey)
	params.Set("qbase64", base64.StdEncoding.EncodeToString([]byte(req.Query)))
	params.Set("size", fmt.Sprintf("%d", req.Size))
	params.Set("page", fmt.Sprintf("%d", req.Page))
	params.Set("fields", strings.TrimSpace(req.Fields))
	params.Set("full", fmt.Sprintf("%t", req.Full))
	u.RawQuery = params.Encode()

	resp, err := h.doRequest(ctx, http.MethodGet, u.String(), nil, "", "", "FOFA")
	if err != nil {
		return SpaceSearchResponse{}, err
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return SpaceSearchResponse{}, upstreamSpaceSearchError(fmt.Sprintf("FOFA 返回非 2xx: %d", resp.StatusCode))
	}
	var apiResp fofaAPIResponse
	if err := json.NewDecoder(resp.Body).Decode(&apiResp); err != nil {
		return SpaceSearchResponse{}, upstreamSpaceSearchError("解析 FOFA 响应失败: " + err.Error())
	}
	if apiResp.Error {
		...
		return SpaceSearchResponse{}, upstreamSpaceSearchError(msg)
	}
	...
	return SpaceSearchResponse{
		Provider: provider, Query: req.Query, Size: firstPositive(apiResp.Size, req.Size), Page: firstPositive(apiResp.Page, req.Page),
		Total: apiResp.Total, Fields: fields, ResultsCount: len(results), Results: results,
	}, nil
}

func (h *FofaHandler) searchExternalProvider(ctx context.Context, provider string, req SpaceSearchRequest) (SpaceSearchResponse, error) {
	apiKey := h.resolveAPIKey(provider)
	if apiKey == "" {
		envKey := map[string]string{"zoomeye": "ZOOMEYE_API_KEY", "quake": "QUAKE_API_KEY", "shodan": "SHODAN_API_KEY"}[provider]
		return SpaceSearchResponse{}, &SpaceSearchError{
			Status: http.StatusBadRequest, Message: providerDisplayName(provider) + " 未配置：请在资产配置中填写 API Key，或设置环境变量 " + envKey,
			Need: []string{provider + ".api_key"}, EnvKey: []string{envKey},
		}
	}
	switch provider {
	case "zoomeye":
		return h.searchZoomEye(ctx, req, apiKey)
	case "quake":
		return h.searchQuake(ctx, req, apiKey)
	case "shodan":
		return h.searchShodan(ctx, req, apiKey)
	default:
		return SpaceSearchResponse{}, &SpaceSearchError{Status: http.StatusBadRequest, Message: "provider 不支持"}
	}
}

func (h *FofaHandler) searchZoomEye(ctx context.Context, req SpaceSearchRequest, apiKey string) (SpaceSearchResponse, error) {
	u, err := url.Parse(h.resolveBaseURL("zoomeye"))
	if err != nil {
		return SpaceSearchResponse{}, &SpaceSearchError{Status: http.StatusInternalServerError, Message: "ZoomEye base_url 无效: " + err.Error()}
	}
	body := map[string]interface{}{"qbase64": base64.StdEncoding.EncodeToString([]byte(req.Query)), "page": req.Page, "pagesize": req.Size}
	...
	if err := h.doJSONRequest(ctx, http.MethodPost, u.String(), apiKey, "API-KEY", body, &apiResp, "ZoomEye"); err != nil {
		return SpaceSearchResponse{}, err
	}
	...
	fields := splitAndCleanCSV(req.Fields)
	return SpaceSearchResponse{
		Provider: "zoomeye", Query: firstNonEmptySpaceSearchValue(apiResp.Query, req.Query), Size: firstPositive(apiResp.PageSize, req.Size), Page: firstPositive(apiResp.Page, req.Page), Total: apiResp.Total,
		Fields: fields, ResultsCount: len(apiResp.Data), Results: projectRows(apiResp.Data, fields),
	}, nil
}

func (h *FofaHandler) searchQuake(ctx context.Context, req SpaceSearchRequest, apiKey string) (SpaceSearchResponse, error) {
	u, err := url.Parse(h.resolveBaseURL("quake"))
	if err != nil {
		return SpaceSearchResponse{}, &SpaceSearchError{Status: http.StatusInternalServerError, Message: "Quake base_url 无效: " + err.Error()}
	}
	fields := splitAndCleanCSV(req.Fields)
	body := map[string]interface{}{"query": req.Query, "size": req.Size, "start": (req.Page - 1) * req.Size, "latest": req.Full}
	if len(fields) > 0 {
		body["include"] = fields
	}
	var apiResp struct {
	...
	if err := h.doJSONRequest(ctx, http.MethodPost, u.String(), apiKey, "X-QuakeToken", body, &apiResp, "Quake"); err != nil {
		return SpaceSearchResponse{}, err
	}
	...
	return SpaceSearchResponse{
		Provider: "quake", Query: req.Query, Size: req.Size, Page: req.Page, Total: total,
		Fields: fields, ResultsCount: len(apiResp.Data), Results: projectRows(apiResp.Data, fields),
	}, nil
}
...
func (h *FofaHandler) searchShodan(ctx context.Context, req SpaceSearchRequest, apiKey string) (SpaceSearchResponse, error) {
	baseURL := strings.TrimRight(h.resolveBaseURL("shodan"), "/") + "/shodan/host/search"
	u, err := url.Parse(baseURL)
	if err != nil {
		return SpaceSearchResponse{}, &SpaceSearchError{Status: http.StatusInternalServerError, Message: "Shodan base_url 无效: " + err.Error()}
	}
	...
	for i := 0; i < pagesNeeded; i++ {
		...
		apiResp.Matches = nil
		apiResp.Error = ""
		if err := h.doJSONRequest(ctx, http.MethodGet, pageURL.String(), "", "", nil, &apiResp, "Shodan"); err != nil {
			return SpaceSearchResponse{}, err
		}
		if strings.TrimSpace(apiResp.Error) != "" {
			return SpaceSearchResponse{}, upstreamSpaceSearchError(apiResp.Error)
		}
...
func writeSpaceSearchError(c *gin.Context, err error) {
	status := http.StatusBadGateway
	payload := gin.H{"error": "空间测绘搜索失败"}
	var searchErr *SpaceSearchError
	if errors.As(err, &searchErr) && searchErr != nil {
		if searchErr.Status > 0 {
			status = searchErr.Status
		}
		payload["error"] = searchErr.Message
		if len(searchErr.Need) > 0 {
			payload["need"] = searchErr.Need
		}
		if len(searchErr.EnvKey) > 0 {
			payload["env_key"] = searchErr.EnvKey
		}
	} else if err != nil {
...
func upstreamSpaceSearchError(message string) *SpaceSearchError {
	return &SpaceSearchError{Status: http.StatusBadGateway, Message: message}
}

func (h *FofaHandler) doRequest(ctx context.Context, method, endpoint string, body interface{}, apiKey, headerName, label string) (*http.Response, error) {
	var reader *strings.Reader
	if body != nil {
		encoded, err := json.Marshal(body)
		if err != nil {
			return nil, &SpaceSearchError{Status: http.StatusInternalServerError, Message: "创建请求失败: " + err.Error()}
		}
	...
	request.Header.Set("User-Agent", "Provena/1.7.4")
	...
	if err != nil {
		status, message, timeout := safeSpaceSearchRequestError(label, err)
		if h != nil && h.logger != nil {
			h.logger.Warn("请求空间测绘搜索失败", zap.String("provider", label), zap.Bool("timeout", timeout), zap.String("error_type", fmt.Sprintf("%T", err)))
		}
		return nil, &SpaceSearchError{Status: status, Message: message}
	}
	return resp, nil
}

func (h *FofaHandler) doJSONRequest(ctx context.Context, method, endpoint, apiKey, headerName string, body interface{}, out interface{}, label string) error {
	resp, err := h.doRequest(ctx, method, endpoint, body, apiKey, headerName, label)
	if err != nil {
		return err
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return upstreamSpaceSearchError(fmt.Sprintf("%s 返回非 2xx: %d", label, resp.StatusCode))
	}
	if err := json.NewDecoder(resp.Body).Decode(out); err != nil {
		return upstreamSpaceSearchError("解析 " + label + " 响应失败: " + err.Error())
	}
	return nil
}

func safeSpaceSearchRequestError(label string, err error) (int, string, bool) {
	status, message, timeout := safeFofaRequestError(err)
	message = strings.Replace(message, "FOFA", label, 1)
