...
import (
	...
	workflowrunner "github.com/chobits02/provena/internal/workflow"
...
func (h *WorkflowHandler) GenerateDraft(c *gin.Context) {
	...
	if llmErr != nil {
		result, llmErr = workflowrunner.GenerateDraftFromNaturalLanguage(c.Request.Context(), draftReq)
		if llmErr != nil {
			c.JSON(http.StatusBadGateway, gin.H{"error": "工作流生成失败: " + llmErr.Error()})
			return
		}
		if result != nil && result.Generator == "deterministic" {
			llmErr = nil
		}
	}
	if h.audit != nil {
