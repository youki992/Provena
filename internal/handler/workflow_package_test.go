...
import (
	...
	workflowpkg "github.com/chobits02/provena/internal/workflow/package"
...
func TestWorkflowHandlerGenerateDraft(t *testing.T) {
	...
	if resp.Result.Generator != "deterministic" {
		t.Fatalf("unexpected generator: %#v", resp.Result.Generator)
	}
	if resp.Result.Graph.Config["schema_version"] == nil {
		t.Fatalf("missing workflow graph config: %#v", resp.Result.Graph.Config)
	}
}
