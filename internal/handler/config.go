...
import (
	...
	"github.com/chobits02/provena/internal/agents"
...
type PacketCaptureRestarter interface {
	ReconcilePacketCapture(cfg config.PacketCaptureConfig) error
}
...
type ConfigHandler struct {
	...
	packetCaptureRestarter     PacketCaptureRestarter     // 本地抓包代理启停与 CA 重载
	audit                      *audit.Service
	db                         *database.DB
	logger                     *zap.Logger
	mu                         sync.RWMutex
...
func (h *ConfigHandler) SetPacketCaptureRestarter(restarter PacketCaptureRestarter) {
	h.packetCaptureRestarter = restarter
}

// ApplyPacketCaptureConfig persists and immediately reconciles the local proxy.
func (h *ConfigHandler) ApplyPacketCaptureConfig(cfg config.PacketCaptureConfig) error {
	h.mu.Lock()
	h.config.PacketCapture = cfg
	if err := h.saveConfig(); err != nil {
		h.mu.Unlock()
		return err
	}
	restarter := h.packetCaptureRestarter
	h.mu.Unlock()
	if restarter != nil {
		return restarter.ReconcilePacketCapture(cfg)
	}
	return nil
}
...
type GetConfigResponse struct {
	...
	PiAgent       config.PiAgentConfig       `json:"pi_agent"`
	...
	PacketCapture config.PacketCaptureConfig `json:"packet_capture"`
}
...
type ToolConfigInfo struct {
	...
	IsLocal     bool                   `json:"is_local,omitempty"`     // 是否为本地 CLI 工具
...
func (h *ConfigHandler) GetConfig(c *gin.Context) {
	...
	c.JSON(http.StatusOK, GetConfigResponse{
		...
		PiAgent:       h.config.PiAgent,
		Hitl:          h.config.Hitl,
		Knowledge:     h.config.Knowledge,
		C2:            h.config.C2.Public(),
		PacketCapture: h.config.PacketCapture,
...
type UpdateConfigRequest struct {
	...
	PiAgent       *config.PiAgentConfig       `json:"pi_agent,omitempty"`
	Hitl          *config.HitlConfig          `json:"hitl,omitempty"`
	Knowledge     *config.KnowledgeConfig     `json:"knowledge,omitempty"`
	Robots        *config.RobotsConfig        `json:"robots,omitempty"`
	MultiAgent    *config.MultiAgentAPIUpdate `json:"multi_agent,omitempty"`
	C2            *config.C2APIUpdate         `json:"c2,omitempty"`
	PacketCapture *config.PacketCaptureConfig `json:"packet_capture,omitempty"`
}
...
func (h *ConfigHandler) UpdateConfig(c *gin.Context) {
	...
	if req.OpenAI != nil {
		globalSystemPrompt := h.config.OpenAI.GlobalSystemPrompt
		h.config.OpenAI = *req.OpenAI
		h.config.OpenAI.GlobalSystemPrompt = globalSystemPrompt
	...
	if req.PacketCapture != nil {
		h.config.PacketCapture = *req.PacketCapture
	}
	...
	if req.PiAgent != nil {
		h.config.PiAgent = *req.PiAgent
		h.logger.Info("更新 Pi Agent 配置",
			zap.Bool("enabled", h.config.PiAgent.Enabled),
			zap.String("model", h.config.PiAgent.Model),
			zap.String("thinking", h.config.PiAgent.Thinking),
			zap.Int("tools", len(h.config.PiAgent.Tools)),
		)
	}
...
func (h *ConfigHandler) ApplyConfig(c *gin.Context) {
	...
	// 抓包代理监听与 CA 变更需要立即重载，不必重启主 API。
	h.mu.RLock()
	packetCaptureRt := h.packetCaptureRestarter
	packetCaptureCfg := h.config.PacketCapture
	h.mu.RUnlock()
	if packetCaptureRt != nil {
		if err := packetCaptureRt.ReconcilePacketCapture(packetCaptureCfg); err != nil {
			h.logger.Error("抓包代理配置应用失败", zap.Error(err))
			c.JSON(http.StatusInternalServerError, gin.H{"error": "抓包代理启动失败: " + err.Error()})
			return
		}
	}
...
func (h *ConfigHandler) saveConfig() error {
	...
	updatePiAgentConfig(root, h.config.PiAgent)
	...
	updatePacketCaptureConfig(root, h.config.PacketCapture)
...
func updatePiAgentConfig(doc *yaml.Node, cfg config.PiAgentConfig) {
	root := doc.Content[0]
	piNode := ensureMap(root, "pi_agent")
	setBoolInMap(piNode, "enabled", cfg.Enabled)
	setStringInMap(piNode, "command", cfg.Command)
	setStringInMap(piNode, "provider", cfg.Provider)
	setStringInMap(piNode, "model", cfg.Model)
	setStringInMap(piNode, "thinking", cfg.Thinking)
	setIntInMap(piNode, "harness_max_activities", cfg.HarnessMaxActivities)
	setIntInMap(piNode, "harness_worker_count", cfg.HarnessWorkerCount)
	setIntInMap(piNode, "activity_timeout_seconds", cfg.ActivityTimeoutSeconds)
	setStringInMap(piNode, "working_dir", cfg.WorkingDir)
	setBoolInMap(piNode, "no_session", cfg.NoSession)
	setStringInMap(piNode, "session_dir", cfg.SessionDir)
	setFlowStringSliceInMap(piNode, "tools", cfg.Tools)
	setBoolInMap(piNode, "no_context_files", cfg.NoContextFiles)
	setBoolInMap(piNode, "no_skills", cfg.NoSkills)
	setBoolInMap(piNode, "no_prompt_templates", cfg.NoPromptTemplates)
	setBoolInMap(piNode, "no_extensions", cfg.NoExtensions)
	supervisorNode := ensureMap(piNode, "supervisor")
	setBoolInMap(supervisorNode, "enabled", cfg.Supervisor.Enabled)
	setStringInMap(supervisorNode, "command", cfg.Supervisor.Command)
	setStringInMap(supervisorNode, "provider", cfg.Supervisor.Provider)
	setStringInMap(supervisorNode, "model", cfg.Supervisor.Model)
	setStringInMap(supervisorNode, "thinking", cfg.Supervisor.Thinking)
	setIntInMap(supervisorNode, "interval_rounds", cfg.Supervisor.IntervalRounds)
	setIntInMap(supervisorNode, "timeout_seconds", cfg.Supervisor.TimeoutSeconds)
	setStringInMap(supervisorNode, "prompt", cfg.Supervisor.Prompt)
}

func updatePacketCaptureConfig(doc *yaml.Node, cfg config.PacketCaptureConfig) {
	root := doc.Content[0]
	node := ensureMap(root, "packet_capture")
	setBoolInMap(node, "enabled", cfg.Enabled)
	setStringInMap(node, "host", cfg.Host)
	setIntInMap(node, "port", cfg.Port)
	setIntInMap(node, "max_body_bytes", cfg.MaxBodyBytes)
	setIntInMap(node, "retention_days", cfg.RetentionDays)
	setStringInMap(node, "ca_path", cfg.CAPath)
	setStringInMap(node, "ca_key_path", cfg.CAKeyPath)
	setBoolInMap(node, "send_full_to_pi", cfg.SendFullToPi)
	setFlowStringSliceInMap(node, "allowed_cidrs", cfg.AllowedCIDRs)
}
...
type builtinToolTestRequest struct {
	Arguments map[string]interface{} `json:"arguments"`
}

// TestBuiltinTool invokes one registered builtin tool with caller-supplied JSON.
// It intentionally goes through mcp.Server.CallTool so authorization, execution
// history, monitoring, cancellation and result guards remain identical to an
// agent-originated call.
func (h *ConfigHandler) TestBuiltinTool(c *gin.Context) {
	...
	if strings.Contains(toolName, "::") {
		c.JSON(http.StatusBadRequest, gin.H{"ok": false, "error": "该接口只允许测试内置工具，外部 MCP 工具请在外部 MCP 验证窗口中测试"})
		return
	}

	var req builtinToolTestRequest
	if err := c.ShouldBindJSON(&req); err != nil {
	...
	h.mu.RUnlock()
	if mcpServer == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"ok": false, "tool": toolName, "error": "MCP 服务不可用"})
		return
	}
	...
	defer cancel()
	result, executionID, err := mcpServer.CallTool(ctx, toolName, req.Arguments)
	if err != nil {
