...
import (
	...
	"github.com/chobits02/provena/internal/agentfinalizer"
...
func (h *AgentHandler) EinoSingleAgentLoopStream(c *gin.Context) {
	// Keep the historical endpoint as a compatibility alias. New deployments
	// use Pi for single-agent streaming unless pi_agent.enabled is explicitly
	// disabled in configuration.
	if h != nil && h.config != nil && h.config.PiAgent.Enabled {
		h.PiSingleAgentLoopStream(c)
		return
	}
