...
func setupTestRouter() (*gin.Engine, *ExternalMCPHandler, string) {
	...
	api.POST("/external-mcp/:name/validate", handler.ValidateExternalMCP)
	api.POST("/external-mcp/:name/test", handler.TestExternalMCPTool)
...
func TestExternalMCPHandler_ValidateExternalMCP_RejectsInvalidConfig(t *testing.T) {
	router, handler, _ := setupTestRouter()
	handler.manager.AddOrUpdateConfig("broken", config.ExternalMCPServerConfig{
		Type:              "unsupported",
		ExternalMCPEnable: true,
	})

	req := httptest.NewRequest("POST", "/api/external-mcp/broken/validate", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
	...
	if response["ok"] != false {
		t.Fatalf("期望 ok=false，实际 %#v", response["ok"])
	}
	if !strings.Contains(response["error"].(string), "不支持的传输模式") {
		t.Fatalf("期望返回明确的传输模式错误，实际 %q", response["error"])
	}
}

func TestExternalMCPHandler_AddOrUpdateExternalMCP_NormalizesEnabledBeforeManager(t *testing.T) {
	router, handler, configPath := setupTestRouter()
	defer cleanupTestConfig(configPath)

	body := []byte(`{"config":{"command":"missing-mcp-server-for-test","description":"test"}}`)
	req := httptest.NewRequest("PUT", "/api/external-mcp/sync-enabled", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
	...
	stored, ok := handler.manager.GetConfigs()["sync-enabled"]
	if !ok {
		t.Fatal("管理器中未找到已保存的 MCP 配置")
	}
	if !stored.ExternalMCPEnable {
		t.Fatal("保存配置后管理器仍为禁用状态")
	}
}
