...
func chatRequestAgentMode(req *ChatRequest, source string) string {
	if strings.HasPrefix(strings.TrimSpace(source), "pi_agent") {
		return "pi_single"
	}
...
func (h *AgentHandler) prepareMultiAgentSession(req *ChatRequest, c *gin.Context, source string) (*multiAgentPrepared, error) {
	...
	isPiRequest := strings.HasPrefix(strings.TrimSpace(source), "pi_agent")
	var agentHistoryMessages []agent.ChatMessage
	var historyErr error
	if isPiRequest {
		// Pi/Harness deliberately does not read conversation history. A fresh
		// Decide/Execute activity receives only the current request; unfinished
		// work is resumed through the external FGS graph, never through chat
		// messages or a hidden model trace.
		agentHistoryMessages = nil
	} else {
		agentHistoryMessages, historyErr = h.loadHistoryFromAgentTrace(conversationID)
		if historyErr != nil {
	...
	if webshellID != "" {
		...
		if !isPiRequest && req.Role != "" && req.Role != "默认" && h.config != nil && h.config.Roles != nil {
	...
	} else if !isPiRequest && req.Role != "" && req.Role != "默认" && h.config != nil && h.config.Roles != nil {
		if role, exists := h.config.Roles[req.Role]; exists && role.Enabled {
			// Roles remain available only to legacy Eino callers. Pi/Harness
			// never enters this branch and therefore cannot inherit role prompts
			// or role-scoped tool lists.
			if role.UserPrompt != "" {
				finalMessage = role.UserPrompt + "\n\n" + req.Message
			}
	...
	// Paths are useful for tools, but are not sufficient for the initial
	// Decide/Execute turn. Inject a bounded text preview so an attached asset
	// list or request data can define the task immediately.
	finalMessage = appendAttachmentContentsToMessage(finalMessage, req.Attachments, savedPaths)
