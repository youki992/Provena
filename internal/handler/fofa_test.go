...
func TestFofaSearchUsesEmailAndAPIKey(t *testing.T) {
	...
	if receivedEmail != "env@example.com" {
		t.Fatalf("FOFA request email = %q, want env@example.com", receivedEmail)
	}
...
func TestFofaSearchRequiresEmailAndAPIKey(t *testing.T) {
	gin.SetMode(gin.TestMode)
	t.Setenv("FOFA_EMAIL", "")
	t.Setenv("FOFA_API_KEY", "")
	h := NewFofaHandler(&config.Config{FOFA: config.FofaConfig{APIKey: "configured-key"}}, zap.NewNop())
	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/fofa/search", strings.NewReader(`{"query":"domain=\"example.com\""}`))
	ctx.Request.Header.Set("Content-Type", "application/json")
	...
	for _, want := range []string{"fofa.email", "fofa.api_key", "FOFA_EMAIL", "FOFA_API_KEY"} {
		if !strings.Contains(body, want) {
			t.Fatalf("missing configuration hint %q in %s", want, body)
		}
	}
}
...
func TestExtractInfoCollectJSONObject(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "plain json",
			in:   `{"query":"title:\"Provena\"","warnings":[]}`,
			want: `{"query":"title:\"Provena\"","warnings":[]}`,
		},
		{
			name: "fenced json",
			in:   "```json\n{\"query\":\"product:nginx\"}\n```",
