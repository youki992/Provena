...
func (h *AgentHandler) SetPacketCaptureService(service *packetcapture.Service) {
	h.packetCapture = service
}
...
type ChatRequest struct {
	...
	PacketGroupIDs []string `json:"packetGroupIds,omitempty"`
}
...
// appendAttachmentContentsToMessage makes uploaded files usable by Pi/Harness
// immediately.  A server path alone is not model-visible input: the model may
// choose to call a tool later, but it cannot know that an attached asset list
// contains the actual task targets.  Keep the injected preview bounded and
// leave binary files as explicit tool-readable paths.
func appendAttachmentContentsToMessage(msg string, attachments []ChatAttachment, savedPaths []string) string {
	if len(attachments) == 0 {
		return msg
	}
	...
	b.WriteString(msg)
	b.WriteString("\n\n[用户附件内容：必须先读取并理解]\n")
	b.WriteString("以下内容来自用户本轮明确上传的附件。附件可能定义任务目标、范围或输入数据；在 Decide/Execute 前先解析它。若预览不完整或是二进制文件，必须使用可用的文件读取/分析工具读取对应路径，不得假设附件为空。\n")
	for i, a := range attachments {
		...
		if path != "" {
			b.WriteString("路径: " + path + "\n")
		}
		if path == "" {
			b.WriteString("附件没有可读取的服务器路径。\n")
			continue
		}
		raw, err := os.ReadFile(path)
		if err != nil {
			b.WriteString("服务端预览读取失败，请使用文件工具读取；错误: " + err.Error() + "\n")
			continue
		}
		if len(raw) == 0 {
			b.WriteString("文件为空。\n")
		...
		if !utf8.Valid(preview) || bytes.IndexByte(preview, 0) >= 0 {
			b.WriteString(fmt.Sprintf("二进制文件（%d 字节），请调用文件/图片分析工具，不要把路径当作已读内容。\n", len(raw)))
			continue
		}
		b.WriteString("文本预览:\n")
		b.Write(preview)
		if truncated {
			b.WriteString(fmt.Sprintf("\n[预览已截断，完整文件 %d 字节；必须继续读取剩余内容或按需解析]\n", len(raw)))
		}
		b.WriteByte('\n')
	}
	return b.String()
