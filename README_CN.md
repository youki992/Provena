...
**Provena 是 AI 原生网络安全的智能执行中枢——让意图转化为受治理的行动，让证据沉淀为运营记忆，并让每次行动优化下一次行动。**

Provena 将规划、执行、人工监督、证据与复盘连接在同一个可审计工作空间中。项目基于 Go 构建，融合 Eino 智能体、MCP 原生工具、RAG 知识、可视化工作流以及攻击链建模与分析能力，面向已获得明确授权的安全任务。

**从这里开始：** [快速上手](#快速上手一条命令部署) · [中文文档](docs/zh-CN/README.md) · [安全加固](docs/zh-CN/security-hardening.md)

> [!IMPORTANT]
> 仅可对自有系统或已获得明确授权的目标使用 Provena。在共享或生产环境启用高风险工具、WebShell 或 C2 前，请先阅读[安全模型](docs/zh-CN/security-model.md)和[安全加固指南](docs/zh-CN/security-hardening.md)。
...
以下能力**默认关闭，需要显式开启**；全新安装不会启动 C2 监听器，也不会把 WebShell 工具暴露给智能体。

- 🐚 **WebShell 管理**：提供连接管理、虚拟终端、文件操作和 AI 辅助工作流。用 `webshell.enabled: true` 开启。
- 📡 **内置 C2**：提供监听器、加密 Beacon、会话、任务队列、Payload 辅助和实时事件。用 `c2.enabled: true` 开启。
...
- **Burp Suite 插件**：`plugins/burp-suite/provena-burp-extension/`  
  构建产物：`plugins/burp-suite/provena-burp-extension/dist/provena-burp-extension.jar`  
  说明文档：`plugins/burp-suite/provena-burp-extension/README.zh-CN.md`
- **浏览器扩展（Chrome / Edge）**：`plugins/browser-extension/provena-browser-extension/`  
  在 DevTools 中捕获 Network 流量并发送到 Provena 进行 AI 辅助安全测试，能力与 Burp 插件对齐。  
  安装：`chrome://extensions/` → 加载已解压 → F12 → **Provena** 标签页  
  打包产物：`plugins/browser-extension/provena-browser-extension/dist/provena-browser-extension.zip`  
  说明文档：`plugins/browser-extension/provena-browser-extension/README.zh-CN.md`
...
**网络默认：** `run.sh` 会以 **`--https`** 并传入项目根 **`config.yaml`** 启动（本机自签证书，多路流式场景更稳）。只要明文 HTTP 用 **`./run.sh --http`**。生产环境在 **`config.yaml`** 的 **`server.tls_cert_path` / `server.tls_key_path`** 配正式证书（见文件内注释）。手动启动可加 **`--https`** 或环境变量 **`PROVENA_HTTPS=1`**；`-config` 写错时程序会在终端提示正确写法。
...
> **Web 控制台需要 `-tags webconsole`。** 默认的 `go build ./cmd/provena` 产出纯 CLI 版：
> 没有 `serve` 子命令、不注册任何 HTTP 路由、不会绑定端口。CLI 命令
> （`run` / `doctor` / `init` / `config` / `version`）两种构建完全一致。

### 命令行接口

同一个二进制同时也是一个小型 CLI，可在启动服务前先做自检：

```bash
provena init                # 由 config.example.yaml 生成 config.yaml
provena doctor              # 检查配置、模型凭证、pi/python、MCP 服务器
provena config validate     # 校验配置文件
provena chat -t <目标>       # 交互式会话：多轮上下文、可打断、可恢复
provena run -t <目标>        # 对单个目标执行有界的无头测试
provena serve               # 启动 Web 控制台与 MCP 端点（需 webconsole 构建）
provena version
```

所有命令都支持 `-config <path>`（默认 `config.yaml`）。带 `webconsole` 标签构建时，
直接传入裸参数（例如 `provena --https -config config.yaml`）会按 `provena serve` 处理，
因此既有的启动脚本、systemd 单元和容器入口都无需修改；纯 CLI 构建下裸参数会明确报错
并提示如何重建。

### 交互式会话

`provena chat` 在整个对话期间只保持一个 Pi 进程，因此模型能看到之前的轮次，并且可以
随时打断与恢复：
...
`provena run` 复用与控制台完全相同的智能体核心，但不需要浏览器。每次运行把状态保存在
只追加的 Fact/Intent 图中，并输出报告：
...
在消耗 token 之前，用 `--dry-run` 确认模型通道、`pi` 运行时和工具注册表都已就绪，是最快的方式。
...
├── skills/              # Agent Skills 目录（SKILL.md + 可选文件；示例 provena-eino-demo）
...
Provena 现已加入 [404星链计划](https://github.com/knownsec/404StarLink)
...
<img src="./images/wechat-group-provena-qr.jpg" alt="Provena 微信群二维码" width="280">
...
Provena 采用 **Apache License 2.0** 开源许可。  
...
Provena 是一个专业的安全测试平台，旨在帮助安全研究人员、渗透测试人员和IT专业人员在**获得明确授权**的情况下进行安全评估和漏洞研究。
