...
| | 能力 | 说明 |
| :---: | --- | --- |
| 🧠 | **Pi harness 驱动** | 模型只负责决策，工具由 Provena 执行；每一轮都可回放 |
| 🕸️ | **只追加的 FGS 图** | 观察记进 fact/intent 图，结论必须挂证据，自我修正也留痕 |
| 🧰 | **100+ 工具配置** | YAML 声明、MCP 调用；本地扫描器与远端 MCP 一视同仁 |
| 📄 | **三种报告格式** | `md` / `json` / `sarif`，SARIF 可直接接进 CI |
| 🙋 | **HITL 审批** | 高风险动作先审批，可由人工或审计 Agent 接管 |
| 📦 | **单文件交付** | Windows / Linux 一个二进制，无服务端、无控制台 |
