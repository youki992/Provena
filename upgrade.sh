...
# Provena GitHub one-click upgrade script (Release/Tag)
#
# Default preserves:
# - config.yaml
# - data/
...
Options:
  ...
  --force-stop         If no process matching current directory is found, also stop
                        any provena processes (use with caution).
...
stop_service() {
  ...
  # If nothing is found and --force-stop is enabled, stop all provena processes.
