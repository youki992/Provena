...
The web console is an opt-in build, so a source run needs the `webconsole` tag. An untagged build is CLI-only (`run` / `chat` / `doctor`) and `serve` fails with a message explaining how to rebuild.

The frontend is static. Most JS/CSS/template changes only require a browser refresh.

Routes are registered in `setupRoutes` in `internal/app/routes.go` (that file carries the `webconsole` tag; a CLI-only build swaps in the same-signature empty implementation from `routes_stub.go`, which registers nothing). Dependencies are constructed in `New` in `internal/app/app.go`, and the parameter lists on both sides must stay identical.
