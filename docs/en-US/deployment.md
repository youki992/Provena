...
Provena can run as a local testing tool, an internal team service, or a production red-team platform. Treat it as a high-privilege security system: it can execute commands, call MCP tools, manage WebShell connections, and optionally run C2 listeners.
...
The web console is an **opt-in build**. Deploy it with the `webconsole` tag — a build without the tag has no `serve` subcommand, registers no routes and binds no port:

```bash
go build -tags webconsole -o provena ./cmd/provena
./provena --config config.yaml          # in this build a bare flag list means serve
```

Headless-only deployments (`provena run` / `chat` / `doctor` / `config validate`) need no tag:

```bash
go build -o provena ./cmd/provena       # CLI-only
```

A console build still needs `web/templates`, `web/static`, and the runtime resource directories.
...
- App wiring: `internal/app/app.go`; route table: `internal/app/routes.go`
