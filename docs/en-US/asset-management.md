...
1. Configure the relevant API in the configuration file or under **System Settings → Asset Management**. FOFA requires both the account email and API key; you may use `FOFA_EMAIL` and `FOFA_API_KEY`. The other providers support `ZOOMEYE_API_KEY`, `QUAKE_API_KEY`, and `SHODAN_API_KEY`.
