...
Provena uses MCP as the primary tool protocol. Tools can be built-in, YAML-backed, Skill-local, or provided by external MCP servers.
...
The management page provides Validate and Save & Validate actions. Validation performs a real MCP handshake and `tools/list`, then shows the discovered tools, input schemas, and duration. You can select a tool and send JSON arguments for a real call. Invalid configuration, connection failures, handshake failures, and an empty `tools/list` are reported separately.

Supported fields include `type` (`stdio`/`http`/`sse`), `command`, `args`, `env`, `url`, `headers`, `timeout`, `max_retries`, `keep_alive`, `terminate_duration`, and `autoApprove`. The legacy `transport` field remains readable; new configurations are saved with `type`.
