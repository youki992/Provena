...
Provena exposes built-in OpenAPI docs:
...
- External MCP validation: `POST /api/external-mcp/{name}/validate`
- External MCP tool test: `POST /api/external-mcp/{name}/test`
...
- Routes: `internal/app/routes.go` (`setupRoutes`, webconsole build only)
