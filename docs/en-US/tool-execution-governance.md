...
This document describes how Provena governs long-running tools, blocking MCP calls, oversized outputs, cancellation, and context restore. The goal is to preserve standard Agent/Eino tool semantics while preventing tool hangs, context blowups, oversized database records, and unsafe resume behavior.
...
External MCP servers can hang, disconnect, or return failures. Provena uses three protections:
...
Provena uses `multi_agent.eino_middleware.reduction_max_length_for_trunc` as the unified tool result cap. The example configuration uses 50000 bytes.
...
- Provena cannot control how a remote external MCP server collects output internally; it caps results after they enter Provena and protects calls with concurrency limits and circuit breakers.
