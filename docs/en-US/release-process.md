...
go test ./internal/...
go test ./cmd/...
go build -tags webconsole -o provena ./cmd/provena     # release package ships the console
```

The release package must carry the `webconsole` tag: an untagged build is CLI-only — no `serve`
subcommand, no routes, no port — and `deploy/*/START.bat` and `start.sh` fail on it immediately.
To publish a CLI-only package as well, use:

```bash
go build -o provena-cli ./cmd/provena
```
