...
plugins/
  README.md
  burp-suite/provena-burp-extension/
  browser-extension/provena-browser-extension/
```
...
Java extension under `plugins/burp-suite/provena-burp-extension/`. Typical flow: read HTTP from Burp → format prompt → call Provena SSE → show Progress/Final in a Burp tab.

Build: JDK + Gradle/Maven → `bash build-mvn.sh` → `dist/provena-burp-extension.jar`.

## Browser Extension (Chromium DevTools)

MV3 DevTools extension under `plugins/browser-extension/provena-browser-extension/`. Aligned with the Burp plugin: capture Network traffic → HTTP/1.1 prompt → SSE output. Full docs: `README.md` / `README.zh-CN.md` in that directory.

Load unpacked at `chrome://extensions/`, or `bash package.sh` → `dist/provena-browser-extension.zip`.
...
- Burp plugin: `plugins/burp-suite/provena-burp-extension/src/main/java/burp/`
- Browser extension: `plugins/browser-extension/provena-browser-extension/`
