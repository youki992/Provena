...
本文档说明如何通过**个人微信、企业微信、钉钉、飞书、Telegram、Slack、Discord 和 QQ 机器人**使用 Provena，包括平台接入、RBAC 身份绑定、服务账号白名单、命令、验证与故障排查。
...
配置会写入 `config.yaml` 的 `robots` 段，也可在配置文件中直接编辑。通过 Web 点击“应用配置”会自动重启对应连接；直接手工修改 `config.yaml` 时，需要重启 Provena 进程。个人微信绑定成功后程序会自动写入 `robots.wechat` 并重启 iLink 长轮询。
...
下面第三节会按平台写清：在开放平台要做什么、要复制哪些字段、填到 Provena 的哪一栏。
...
**飞书配置简要步骤**：登录 [飞书开放平台](https://open.feishu.cn) → 创建企业自建应用 → 在「凭证与基础信息」中获取 **App ID**、**App Secret** → 在「应用能力」中开通**机器人**并启用相应权限 → **在「事件订阅」中添加事件**（见下）→ 发布应用 → 将 App ID、App Secret 填到 Provena 机器人设置 → 保存。
...
> - Provena 解密并调用 AI → 使用企业微信的 `message/send` 接口**主动发消息给用户**。

**配置概览：**

- 在企业微信管理后台创建或选择一个**自建应用**。
- 在该应用的「接收消息」处配置回调 URL、Token、EncodingAESKey。
- 在 Provena 的 `config.yaml` 中填入：
...
> Provena 使用 `https://qyapi.weixin.qq.com/cgi-bin/message/send` 主动发送 AI 回复。  
> 若企业微信日志或本程序日志中出现 `errcode 60020 not allow to access from your ip`：
>
> - 说明你的服务器出口 IP **没有加入企业微信的 IP 白名单**；  
> - 请在企业微信管理后台中找到该自建应用的**「安全设置 / IP 白名单」**（具体入口可能因版本略有不同），将运行 Provena 的服务器公网 IP（如 `110.xxx.xxx.xxx`）加入白名单；  
> - 保存后等待生效，再次发送消息测试。
>
...
> - 在 Provena Web 端生成二维码 → 用**手机微信**扫码并确认绑定；  
...
6. **在手机微信里测试**  
   打开与 Provena 机器人的**私聊**（绑定后微信内会出现对应会话），发送「帮助」或任意文字测试。
...
| 断网或睡眠后无回复 | 程序会自动重连（约 5～60 秒）；仍无回复可重启 Provena |
...
5. 在 Provena 填入 Bot Token 与 App-Level Token → **应用配置**。
...
4. Provena → **机器人设置** → **Discord** → 填入 Token → **应用配置**。  
...
4. Provena → **机器人设置** → **QQ 机器人** → 填入 App ID、Client Secret。  
5. 测试阶段勾选 **沙箱环境**；正式上线后取消沙箱并发布。  
6. 点击 **应用配置**。

**使用：** 与机器人 C2C 私聊直接发；QQ 群中需 @ 机器人。

> 注意：QQ 官方正逐步推广 Webhook 回调；当前实现使用 WebSocket（与钉钉/飞书类似的长连接模式）。若配置变更后连接未刷新，可重启 Provena 进程。

---

## 四、RBAC 鉴权与机器人命令

平台 Token、签名或长连接凭证只负责证明“消息来自该平台”；真正能执行哪些操作，由 Provena 的 RBAC 决定。每个机器人实例都必须选择一种业务鉴权模式。
...
1. 登录 Provena Web，点击右上角头像 → **绑定机器人账号**。
...
- **个人微信**：在与 Provena 机器人的**私聊**中直接发送即可，**不需要 @**（不支持群聊）。  
...
1. Provena Web 端 → 系统设置 → 机器人设置 → **微信 / iLink** → **生成二维码并绑定**。  
...
2. **在 Provena**：系统设置 → 机器人设置 → 勾选对应平台，粘贴 Client ID/App ID、Client Secret/App Secret → 点击 **应用配置**。  
...
若返回 JSON 中含有 `"reply":"【Provena 机器人命令】..."`，说明命令处理正常。`帮助`、`版本` 和 `身份` 可在未绑定时执行；`列表`、`当前` 和普通 AI 消息会走真实 RBAC，测试用 `platform + user_id` 必须已经绑定，或与服务账号模式的发送者白名单精确匹配。
...
2. **是否已启用**  
   确认「启用微信机器人」已勾选；若刚修改过，可重启 Provena 进程。
...
5. **断网或睡眠后**  
   与钉钉/飞书类似，程序会**自动重连**（约 5～60 秒）；仍无回复可重启 Provena。
...
0. **笔记本合盖睡眠 / 断网后**  
   钉钉、飞书均使用长连接收消息，睡眠或断网后连接会断开。程序会**自动重连**（约 5 秒～60 秒内重试）。唤醒或恢复网络后稍等一会儿再发消息；若仍无反应，可重启 Provena 进程。
...
2. **配置是否已应用**  
   在 Web 端修改后必须点击“应用配置”，程序会自动重启对应连接。若直接手工编辑 `config.yaml`，则需重启 Provena 进程。
