...
`provena serve` 启动时会一并监听抓包代理端口（`packet_capture.port`，默认 `9081`）。
该端口被占用时整个服务起不来，日志是：
...
- 找占用者：Windows `netstat -ano | findstr :9081`，Linux `lsof -i :9081`，
  再按 PID 结束进程，或把 `packet_capture.port` 改到空闲端口。
- 这一轮不需要抓包时直接关掉：
...
`provena run` 会强制关闭抓包代理和 C2，所以无头运行不受这个端口影响。
