...
Provena 内置 OpenAPI 规格和 API 文档页面。启动服务后访问：
...
- `POST /api/external-mcp/:name/validate`
- `POST /api/external-mcp/:name/test`
...
- 路由：`internal/app/routes.go`（`setupRoutes`，仅 `webconsole` 构建）
