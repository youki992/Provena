...
本文说明 Provena 对长时间工具、MCP 阻塞、大输出、取消和恢复上下文的治理策略。目标是让 Agent 保持标准工具语义，同时避免工具卡死、上下文爆炸、数据库膨胀或恢复时重新注入历史大输出。
...
- 外部 MCP 的远端 server 内部如何采集输出不由 Provena 控制；Provena 会在结果进入本系统后统一兜底、限并发和熔断。
