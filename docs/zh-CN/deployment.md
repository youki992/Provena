...
本文说明 Provena 的常见部署方式。生产环境部署前，请先阅读 [安全模型](security-model.md)，确认授权范围、认证、HITL、审计和高风险功能开关。
...
Web 控制台是**可选构建**。部署控制台时带上 `webconsole` 标签 —— 不带标签的产物没有
`serve` 子命令，也不会注册任何路由、绑定任何端口：

```bash
go build -tags webconsole -o provena ./cmd/provena
./provena --config config.yaml          # 此构建下裸参数等价于 serve
```

只跑无头评估（`provena run` / `doctor` / `config validate`）则不需要标签：

```bash
go build -o provena ./cmd/provena       # 纯 CLI 版
```
...
[Service]
Type=simple
WorkingDirectory=/opt/Provena
ExecStart=/opt/Provena/provena --config /opt/Provena/config.yaml
Restart=on-failure
RestartSec=5
...
sudo systemctl enable --now provena
sudo journalctl -u provena -f
```
...
- 服务组装：`internal/app/app.go`；路由表：`internal/app/routes.go`
