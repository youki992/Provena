...
发布包要交付 Web 控制台，所以必须带 `webconsole` 标签。不带标签的产物是纯 CLI 版：
没有 `serve` 子命令、不注册路由、不绑定端口，`deploy/*/START.bat` 与 `start.sh`
在这份产物上会直接报错。若要额外发布一个纯 CLI 包，用：
