const ASSET_PAGE_SIZE_KEY = 'provena.asset_page_size';
const ASSET_SAVED_VIEWS_KEY = 'provena.asset_saved_views';
...
const assetPageState = { page: 1, pageSize: getAssetPageSize(), total: 0, totalPages: 1, items: [], projects: [], projectsLoaded: false, detailIndex: -1, editIndex: -1, detailAsset: null, editAsset: null, selected: new Map(), selectionQuery: '', allMatchingSelected: false, scanMode: 'chat', scanAssets: [], editorTags: [], editorDirty: false, editorBusy: false, editorReturnFocus: null, editorInteractionsReady: false, editorParsedTarget: '', importRows: [], importFileName: '', importBusy: false, importInteractionsReady: false, importReturnFocus: null, listLoading: false, overviewLoading: false };
let assetOverviewDays = 30;
const ASSET_AUTO_REFRESH_INTERVAL_MS = 3000;
let assetAutoRefreshTimer = null;

function isAssetPageActive() {
    const current = typeof window.currentPage === 'function' ? window.currentPage() : '';
    return current === 'asset-library' || current === 'asset-overview';
}

function stopAssetAutoRefresh() {
    if (assetAutoRefreshTimer !== null) {
        clearInterval(assetAutoRefreshTimer);
        assetAutoRefreshTimer = null;
    }
}

function refreshActiveAssetPage() {
    if (document.hidden || !isAssetPageActive()) return;
    if (typeof window.currentPage === 'function' && window.currentPage() === 'asset-library') {
        if (!assetPageState.listLoading) void loadAssets(assetPageState.page, { background: true });
        return;
    }
    void loadAssetOverview({ background: true });
}

function startAssetAutoRefresh() {
    stopAssetAutoRefresh();
    if (document.hidden || !isAssetPageActive()) return;
    assetAutoRefreshTimer = setInterval(refreshActiveAssetPage, ASSET_AUTO_REFRESH_INTERVAL_MS);
}
...
async function loadAssetOverview(options) {
    options = options || {};
    if (assetPageState.overviewLoading) return;
    assetPageState.overviewLoading = true;
    try {
        ...
        if (!options.background && typeof showInlineToast === 'function') showInlineToast(assetT('assets.loadFailed', '加载资产失败') + ': ' + error.message);
    } finally {
        assetPageState.overviewLoading = false;
    }
}
...
// Restore the lightweight asset search carried by links from Pi results.
// Other filters remain untouched so opening the page does not destroy a
// user's current saved/advanced filter state.
function syncAssetFiltersFromLocationHash() {
    const hash = window.location.hash.slice(1);
    const parts = hash.split('?');
    if (parts[0] !== 'asset-library' || parts.length < 2) return;
...
async function loadAssets(page, options) {
    options = options || {};
    if (assetPageState.listLoading) return;
    assetPageState.listLoading = true;
    if (page == null) syncAssetFiltersFromLocationHash();
    ...
    if (!options.background && body) body.innerHTML = '<tr><td colspan="10" class="muted">' + escapeHtml(assetT('common.loading', '加载中...')) + '</td></tr>';
    try {
        ...
        if (assetPageState.page > assetPageState.totalPages) {
            assetPageState.page = assetPageState.totalPages;
            const retryParams = new URLSearchParams(filterParams);
            retryParams.set('page', assetPageState.page);
            retryParams.set('page_size', assetPageState.pageSize);
            const retryResponse = await apiFetch('/api/assets?' + retryParams.toString());
            if (!retryResponse.ok) throw new Error(await retryResponse.text());
            const retryData = await retryResponse.json();
            assetPageState.items = retryData.assets || [];
            assetPageState.total = retryData.total || 0;
            assetPageState.totalPages = retryData.total_pages || 1;
        }
        renderAssetRows();
        ...
        assetPageState.listLoading = false;
    }
}
...
function openAssetVulnerabilities(index) {
    const asset = assetPageState.items[Number(index)];
    if (!asset) return;
    openAssetVulnerabilitiesRecord(asset);
}

function openAssetVulnerabilitiesRecord(asset) {
    if (!asset) return;
...
function openAssetConversationFromDetail() {
    const conversationId = String(assetPageState.detailAsset?.last_scan_conversation_id || '').trim();
    if (!conversationId) return;
    closeAssetDetail();
    if (typeof navigateToConversation === 'function') navigateToConversation(conversationId);
    else window.location.hash = `chat?conversation=${encodeURIComponent(conversationId)}`;
}

function openAssetVulnerabilitiesFromDetail() {
...
function openAssetDetailRecord(asset) {
    ...
    const conversationButton = document.getElementById('asset-detail-conversation');
    if (conversationButton) conversationButton.hidden = !String(asset.last_scan_conversation_id || '').trim();
    const vulnerabilitiesButton = document.getElementById('asset-detail-vulnerabilities');
    if (vulnerabilitiesButton) vulnerabilitiesButton.hidden = !(asset.last_scan_task_id || asset.last_scan_conversation_id);
...
window.startAssetAutoRefresh = startAssetAutoRefresh;
window.stopAssetAutoRefresh = stopAssetAutoRefresh;
...
window.openAssetConversationFromDetail = openAssetConversationFromDetail;
window.openAssetVulnerabilitiesFromDetail = openAssetVulnerabilitiesFromDetail;
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        stopAssetAutoRefresh();
    } else if (isAssetPageActive()) {
        refreshActiveAssetPage();
