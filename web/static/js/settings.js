...
async function loadConfig(loadTools = true, options = {}) {
    ...
    try {
        ...
        // 填充 Pi Agent 配置。Pi 使用独立 CLI 参数；API Key、Base URL、额度仍来自当前 AI 通道。
        const pi = currentConfig.pi_agent || {};
        const piEnabledEl = document.getElementById('pi-agent-enabled');
        if (piEnabledEl) piEnabledEl.checked = pi.enabled !== false;
        const piCommandEl = document.getElementById('pi-agent-command');
        if (piCommandEl) piCommandEl.value = pi.command || 'pi';
        const piModelEl = document.getElementById('pi-agent-model');
        if (piModelEl) piModelEl.value = pi.model || '';
        const piThinkingEl = document.getElementById('pi-agent-thinking');
        if (piThinkingEl) {
            const thinking = String(pi.thinking || '').trim().toLowerCase();
            piThinkingEl.value = ['', 'off', 'minimal', 'low', 'medium', 'high', 'xhigh'].includes(thinking) ? thinking : '';
        }
        const piWorkingDirEl = document.getElementById('pi-agent-working-dir');
        if (piWorkingDirEl) piWorkingDirEl.value = pi.working_dir || '';
        const piSessionDirEl = document.getElementById('pi-agent-session-dir');
        if (piSessionDirEl) piSessionDirEl.value = pi.session_dir || '';
        const piToolsEl = document.getElementById('pi-agent-tools');
        if (piToolsEl) piToolsEl.value = Array.isArray(pi.tools) ? pi.tools.join(',') : '';
        const piNoSessionEl = document.getElementById('pi-agent-no-session');
        if (piNoSessionEl) piNoSessionEl.checked = pi.no_session !== false;
        [
            ['pi-agent-no-context-files', 'no_context_files'],
            ['pi-agent-no-skills', 'no_skills'],
            ['pi-agent-no-prompt-templates', 'no_prompt_templates'],
            ['pi-agent-no-extensions', 'no_extensions']
        ].forEach(([id, key]) => {
            const el = document.getElementById(id);
            if (el) el.checked = pi[key] === true;
        });
        const piSupervisor = pi.supervisor || {};
        const piSupervisorEnabledEl = document.getElementById('pi-agent-supervisor-enabled');
        if (piSupervisorEnabledEl) piSupervisorEnabledEl.checked = piSupervisor.enabled !== false;
        const piSupervisorCommandEl = document.getElementById('pi-agent-supervisor-command');
        if (piSupervisorCommandEl) piSupervisorCommandEl.value = piSupervisor.command || '';
        const piSupervisorProviderEl = document.getElementById('pi-agent-supervisor-provider');
        if (piSupervisorProviderEl) piSupervisorProviderEl.value = piSupervisor.provider || '';
        const piSupervisorModelEl = document.getElementById('pi-agent-supervisor-model');
        if (piSupervisorModelEl) piSupervisorModelEl.value = piSupervisor.model || '';
        const piSupervisorThinkingEl = document.getElementById('pi-agent-supervisor-thinking');
        if (piSupervisorThinkingEl) {
            const thinking = String(piSupervisor.thinking || 'minimal').trim().toLowerCase();
            piSupervisorThinkingEl.value = ['', 'off', 'minimal', 'low', 'medium', 'high', 'xhigh'].includes(thinking) ? (thinking || 'minimal') : 'minimal';
        }
        const piSupervisorIntervalEl = document.getElementById('pi-agent-supervisor-interval');
        if (piSupervisorIntervalEl) piSupervisorIntervalEl.value = Number(piSupervisor.interval_rounds) > 0 ? String(Math.floor(Number(piSupervisor.interval_rounds))) : '1';
        const piSupervisorTimeoutEl = document.getElementById('pi-agent-supervisor-timeout');
        if (piSupervisorTimeoutEl) piSupervisorTimeoutEl.value = Number(piSupervisor.timeout_seconds) > 0 ? String(Math.min(300, Math.floor(Number(piSupervisor.timeout_seconds)))) : '45';
        const piSupervisorPromptEl = document.getElementById('pi-agent-supervisor-prompt');
        if (piSupervisorPromptEl) piSupervisorPromptEl.value = piSupervisor.prompt || '';
        ...
        if (wechatBotAgent) wechatBotAgent.value = wechat.bot_agent || 'Provena/1.0';
...
function renderToolsList() {
    ...
    allTools.forEach(tool => {
        ...
        const isExternalTool = toolState.is_external || tool.is_external;
        const builtinTestButton = !isExternalTool && tool.enabled !== false ? `
                        <button type="button" class="btn-secondary btn-small tool-test-button" data-require-permission="mcp:execute" onclick="openBuiltinToolTestModal(${settingsEscapeJsStringAttr(tool.name)}, event)" title="${settingsEscapeAttr(settingsT('mcp.testBuiltinTool', '试运行内置工具'))}">
                            🧪 ${escapeHtml(settingsT('mcp.testBuiltinToolShort', '试运行'))}
                        </button>` : '';
...
async function applySettings() {
    try {
        ...
        const prevPiAgent = (currentConfig && currentConfig.pi_agent) ? currentConfig.pi_agent : {};
        ...
        const config = {
            ...
            pi_agent: {
                ...prevPiAgent,
                enabled: document.getElementById('pi-agent-enabled')?.checked !== false,
                command: document.getElementById('pi-agent-command')?.value.trim() || 'pi',
                model: document.getElementById('pi-agent-model')?.value.trim() || '',
                thinking: document.getElementById('pi-agent-thinking')?.value || '',
                working_dir: document.getElementById('pi-agent-working-dir')?.value.trim() || '',
                session_dir: document.getElementById('pi-agent-session-dir')?.value.trim() || '',
                tools: (document.getElementById('pi-agent-tools')?.value || '').split(/[,，\s]+/).map(v => v.trim()).filter(Boolean),
                no_session: document.getElementById('pi-agent-no-session')?.checked === true,
                no_context_files: document.getElementById('pi-agent-no-context-files')?.checked === true,
                no_skills: document.getElementById('pi-agent-no-skills')?.checked === true,
                no_prompt_templates: document.getElementById('pi-agent-no-prompt-templates')?.checked === true,
                no_extensions: document.getElementById('pi-agent-no-extensions')?.checked === true,
                supervisor: {
                    ...(prevPiAgent.supervisor && typeof prevPiAgent.supervisor === 'object' ? prevPiAgent.supervisor : {}),
                    enabled: document.getElementById('pi-agent-supervisor-enabled')?.checked !== false,
                    command: document.getElementById('pi-agent-supervisor-command')?.value.trim() || '',
                    provider: document.getElementById('pi-agent-supervisor-provider')?.value.trim() || '',
                    model: document.getElementById('pi-agent-supervisor-model')?.value.trim() || '',
                    thinking: document.getElementById('pi-agent-supervisor-thinking')?.value || 'minimal',
                    interval_rounds: Math.max(1, parseInt(document.getElementById('pi-agent-supervisor-interval')?.value || '1', 10) || 1),
                    timeout_seconds: Math.min(300, Math.max(1, parseInt(document.getElementById('pi-agent-supervisor-timeout')?.value || '45', 10) || 45)),
                    prompt: document.getElementById('pi-agent-supervisor-prompt')?.value.trim() || ''
                }
            },
            ...
            robots: {
                ...
                wechat: {
                    ...
                    bot_agent: document.getElementById('robot-wechat-bot-agent')?.value.trim() || 'Provena/1.0',
...
let currentEditingMCPConfig = null;

function syncExternalMCPEndpointLabel() {
    const kind = document.getElementById('external-mcp-kind')?.value || 'stdio';
    const label = document.getElementById('external-mcp-endpoint-label');
    const input = document.getElementById('external-mcp-endpoint');
    const argsGroup = document.getElementById('external-mcp-args-group');
    const stdioHint = document.getElementById('external-mcp-stdio-hint');
    const envGroup = document.getElementById('external-mcp-env-group');
    const headersGroup = document.getElementById('external-mcp-headers-group');
    const terminateDurationGroup = document.getElementById('external-mcp-terminate-duration-group');
    if (kind === 'http' || kind === 'sse') {
        if (label) label.textContent = settingsT('externalMcpModal.url', 'MCP远程地址');
        if (input) input.placeholder = kind === 'sse'
            ? settingsT('externalMcpModal.sseUrlPlaceholder', 'http://127.0.0.1:8081/sse')
            : settingsT('externalMcpModal.urlPlaceholder', 'https://example.com/mcp');
        if (argsGroup) argsGroup.hidden = true;
        if (stdioHint) stdioHint.hidden = true;
        if (envGroup) envGroup.hidden = true;
        if (headersGroup) headersGroup.hidden = false;
        if (terminateDurationGroup) terminateDurationGroup.hidden = true;
    } else {
        if (label) label.textContent = settingsT('externalMcpModal.path', '工具路径/命令');
        if (input) input.placeholder = settingsT('externalMcpModal.endpointPlaceholder', '例如 /opt/tools/my-mcp 或 python server.py');
        if (argsGroup) argsGroup.hidden = false;
        if (stdioHint) stdioHint.hidden = false;
        if (envGroup) envGroup.hidden = false;
        if (headersGroup) headersGroup.hidden = true;
        if (terminateDurationGroup) terminateDurationGroup.hidden = false;
    }
}

function resetExternalMCPForm() {
    const fields = ['external-mcp-name', 'external-mcp-endpoint', 'external-mcp-args', 'external-mcp-env', 'external-mcp-headers', 'external-mcp-timeout', 'external-mcp-max-retries', 'external-mcp-keep-alive', 'external-mcp-terminate-duration', 'external-mcp-auto-approve', 'external-mcp-description'];
    fields.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    const kind = document.getElementById('external-mcp-kind');
    if (kind) kind.value = 'stdio';
    const errorDiv = document.getElementById('external-mcp-json-error');
    if (errorDiv) { errorDiv.style.display = 'none'; errorDiv.textContent = ''; }
    syncExternalMCPEndpointLabel();
}

function fillExternalMCPForm(name, config) {
    const cfg = config || {};
    const nameEl = document.getElementById('external-mcp-name');
    const kindEl = document.getElementById('external-mcp-kind');
    const endpointEl = document.getElementById('external-mcp-endpoint');
    const descriptionEl = document.getElementById('external-mcp-description');
    const kind = cfg.type || cfg.transport || (cfg.command ? 'stdio' : 'http');
    if (nameEl) nameEl.value = name || '';
    if (kindEl) kindEl.value = ['stdio', 'http', 'sse'].includes(kind) ? kind : 'http';
    if (endpointEl) endpointEl.value = kind === 'stdio' ? (cfg.command || '') : (cfg.url || '');
    const argsEl = document.getElementById('external-mcp-args');
    if (argsEl) argsEl.value = kind === 'stdio' && Array.isArray(cfg.args) ? cfg.args.join('\n') : '';
    const envEl = document.getElementById('external-mcp-env');
    if (envEl) envEl.value = cfg.env && typeof cfg.env === 'object' ? JSON.stringify(cfg.env, null, 2) : '';
    const headersEl = document.getElementById('external-mcp-headers');
    if (headersEl) headersEl.value = cfg.headers && typeof cfg.headers === 'object' ? JSON.stringify(cfg.headers, null, 2) : '';
    const timeoutEl = document.getElementById('external-mcp-timeout');
    if (timeoutEl) timeoutEl.value = Number(cfg.timeout) > 0 ? String(cfg.timeout) : '';
    const retriesEl = document.getElementById('external-mcp-max-retries');
    if (retriesEl) retriesEl.value = Number(cfg.max_retries) > 0 ? String(cfg.max_retries) : '';
    const keepAliveEl = document.getElementById('external-mcp-keep-alive');
    if (keepAliveEl) keepAliveEl.value = Number(cfg.keep_alive) > 0 ? String(cfg.keep_alive) : '';
    const terminateDurationEl = document.getElementById('external-mcp-terminate-duration');
    if (terminateDurationEl) terminateDurationEl.value = Number(cfg.terminate_duration) > 0 ? String(cfg.terminate_duration) : '';
    const autoApproveEl = document.getElementById('external-mcp-auto-approve');
    if (autoApproveEl) autoApproveEl.value = Array.isArray(cfg.autoApprove) ? cfg.autoApprove.join('\n') : '';
    if (descriptionEl) descriptionEl.value = cfg.description || '';
    syncExternalMCPEndpointLabel();
}

function externalMCPFormConfig() {
    const kind = document.getElementById('external-mcp-kind')?.value || 'stdio';
    const endpoint = document.getElementById('external-mcp-endpoint')?.value.trim() || '';
    const argsText = document.getElementById('external-mcp-args')?.value || '';
    const description = document.getElementById('external-mcp-description')?.value.trim() || '';
    const parseObject = (id, label) => {
        const raw = document.getElementById(id)?.value.trim() || '';
        if (!raw) return {};
        let parsed;
        try { parsed = JSON.parse(raw); } catch (error) { throw new Error(`${label}必须是有效 JSON 对象`); }
        if (!parsed || Array.isArray(parsed) || typeof parsed !== 'object') throw new Error(`${label}必须是 JSON 对象`);
        const out = {};
        Object.entries(parsed).forEach(([key, value]) => {
            if (!String(key).trim()) throw new Error(`${label}的键不能为空`);
            out[String(key)] = String(value ?? '');
        });
        return out;
    };
    const parsePositiveInt = (id) => {
        const value = Number.parseInt(document.getElementById(id)?.value || '', 10);
        return Number.isFinite(value) && value > 0 ? value : 0;
    };
    const config = Object.assign({}, currentEditingMCPConfig || {});
    ...
    if (kind === 'stdio') {
        ...
        config.args = argsText.split(/\r?\n/).map(value => value.trim()).filter(Boolean);
        delete config.url;
        config.env = parseObject('external-mcp-env', '环境变量');
        delete config.headers;
    } else {
        config.url = endpoint;
        delete config.args;
        delete config.command;
        config.headers = parseObject('external-mcp-headers', '请求头');
        delete config.env;
    }
    config.timeout = parsePositiveInt('external-mcp-timeout');
    config.max_retries = parsePositiveInt('external-mcp-max-retries');
    config.keep_alive = parsePositiveInt('external-mcp-keep-alive');
    config.terminate_duration = parsePositiveInt('external-mcp-terminate-duration');
    config.autoApprove = (document.getElementById('external-mcp-auto-approve')?.value || '')
        .split(/\r?\n/).map(value => value.trim()).filter(Boolean);
    return config;
}
            ...
            <div class="${cardClass}${selectedClass}" data-mcp-name="${settingsEscapeAttr(name)}"${hasTools ? ` onclick="scrollToExternalMCPTools(${settingsEscapeJsStringAttr(name)}, event)" title="${settingsEscapeAttr(cardClickTitle)}"` : ''}>
                <div class="external-mcp-item-header">
                    ...
                    <div class="external-mcp-item-actions">
                        ${status !== 'connecting' ?
                            `<button class="btn-small" onclick="validateExternalMCP(${settingsEscapeJsStringAttr(name)}, event)" title="${settingsEscapeAttr(statusT('mcp.validateConnection'))}">
                                🧪 ${statusT('mcp.validate')}
                            </button>` : ''}
                        ${status === 'connected' || status === 'disconnected' || status === 'error' || status === 'disabled' ?
                            `<button class="btn-small" id="btn-toggle-${settingsEscapeAttr(name)}" onclick="toggleExternalMCP(${settingsEscapeJsStringAttr(name)}, ${settingsEscapeJsStringAttr(status)}, event)" title="${settingsEscapeAttr(status === 'connected' ? statusT('mcp.stopConnection') : statusT('mcp.startConnection'))}">
                        ...
                        <button class="btn-small" onclick="editExternalMCP(${settingsEscapeJsStringAttr(name)}, event)" title="${settingsEscapeAttr(statusT('mcp.editConfig'))}" ${status === 'connecting' ? 'disabled' : ''}>✏️ ${statusT('common.edit')}</button>
                        <button class="btn-small btn-danger" onclick="deleteExternalMCP(${settingsEscapeJsStringAttr(name)}, event)" title="${settingsEscapeAttr(statusT('mcp.deleteConfig'))}" ${status === 'connecting' ? 'disabled' : ''}>🗑 ${statusT('common.delete')}</button>
                    </div>
                </div>
...
function renderExternalMCPStats(stats) {
    ...
    const connecting = stats.connecting || 0;
    ...
    statsEl.innerHTML = `
        ...
        <span title="${statsT('mcp.connectingCount')}">⏳ ${statsT('mcp.connectingCount')}: <strong>${connecting}</strong></span>
        <span title="${statsT('mcp.errorCount')}">❌ ${statsT('mcp.errorCount')}: <strong>${errors}</strong></span>
        <span title="${statsT('mcp.disconnectedCount')}">⚠️ ${statsT('mcp.disconnectedCount')}: <strong>${disconnected}</strong></span>
    `;
}
...
// 编辑外部MCP
async function editExternalMCP(name, event) {
    ...
    try {
        ...
        delete config.external_mcp_enable;
        currentEditingMCPConfig = config;
        deferModalContent(() => {
            fillExternalMCPForm(name, config);
            document.getElementById('external-mcp-endpoint')?.focus();
        });
    } catch (error) {
...
// 保存外部MCP
async function saveExternalMCP(validateAfterSave = false) {
    if (typeof requirePermission === 'function' && !requirePermission('mcp:write')) return;
    const errorDiv = document.getElementById('external-mcp-json-error');
    const t = (typeof window.t === 'function' ? window.t : function (k, opts) { return k; });
    const name = document.getElementById('external-mcp-name')?.value.trim() || '';
    const kind = document.getElementById('external-mcp-kind')?.value || 'stdio';
    const endpoint = document.getElementById('external-mcp-endpoint')?.value.trim() || '';
    const description = document.getElementById('external-mcp-description')?.value.trim() || '';
    if (!name || !endpoint || !description) {
        errorDiv.textContent = t('externalMcpModal.required');
        errorDiv.style.display = 'block';
        return;
    }
    if ((kind === 'http' || kind === 'sse') && !/^https?:\/\//i.test(endpoint)) {
        errorDiv.textContent = t('externalMcpModal.invalidUrl');
        errorDiv.style.display = 'block';
        return;
    }
    if (currentEditingMCPName && name !== currentEditingMCPName) {
        errorDiv.textContent = t('externalMcpModal.renameUnsupported');
        errorDiv.style.display = 'block';
        return;
    }
    const config = externalMCPFormConfig();
    ...
    try {
        ...
        const savedName = currentEditingMCPName || name;
        closeExternalMCPModal();
        ...
        if (validateAfterSave) {
            await validateExternalMCP(savedName);
        } else {
...
// 删除外部MCP
async function deleteExternalMCP(name, event) {
    if (event) event.stopPropagation();
...
let externalMcpValidationName = '';
let externalMcpValidationTools = [];

function closeExternalMCPValidationModal() {
    closeAppModal('external-mcp-validation-modal');
    externalMcpValidationName = '';
    externalMcpValidationTools = [];
}

function renderExternalMCPValidationSummary(text, success) {
    const el = document.getElementById('external-mcp-validation-summary');
    if (!el) return;
    el.className = `mcp-validation-summary ${success ? 'success' : 'failure'}`;
    el.textContent = text;
}

function renderExternalMCPValidationTools(tools) {
    const list = document.getElementById('external-mcp-validation-tools');
    const panel = document.getElementById('external-mcp-tool-test-panel');
    const select = document.getElementById('external-mcp-test-tool');
    if (!list || !panel || !select) return;
    externalMcpValidationTools = Array.isArray(tools) ? tools : [];
    if (externalMcpValidationTools.length === 0) {
        list.innerHTML = `<div class="password-hint">${escapeHtml(settingsT('mcp.noToolsDiscovered', '握手成功，但服务没有返回任何工具。请检查 MCP Server 是否真正实现 tools/list。'))}</div>`;
        panel.hidden = true;
        return;
    }
    list.innerHTML = externalMcpValidationTools.map(tool => `
        <div class="mcp-validation-tool-row">
            <code>${escapeHtml(tool.name || '')}</code>
            <span>${escapeHtml(tool.description || settingsT('mcp.noDescription', '无描述'))}</span>
        </div>
    `).join('');
    select.innerHTML = externalMcpValidationTools.map((tool, index) =>
        `<option value="${settingsEscapeAttr(tool.name || '')}">${escapeHtml(tool.name || `tool-${index + 1}`)}</option>`
    ).join('');
    panel.hidden = false;
    const result = document.getElementById('external-mcp-test-result');
    if (result) { result.hidden = true; result.textContent = ''; }
    renderExternalMCPTestSchema();
}

function renderExternalMCPTestSchema() {
    const select = document.getElementById('external-mcp-test-tool');
    const schemaEl = document.getElementById('external-mcp-test-schema');
    const argsEl = document.getElementById('external-mcp-test-args');
    if (!select || !schemaEl || !argsEl) return;
    const tool = externalMcpValidationTools.find(item => item.name === select.value);
    const schema = tool && tool.input_schema ? tool.input_schema : null;
    ...
    schemaEl.textContent = props.length > 0
        ? `${settingsT('mcp.testSchemaFields', '参数')}：${props.join(', ')}；${settingsT('mcp.testSchemaRequired', '必填')}：${required.length ? required.join(', ') : settingsT('mcp.none', '无')}`
        : settingsT('mcp.testSchemaEmpty', '该工具未声明参数，可直接使用 {} 测试。');
    if (!argsEl.value.trim() || argsEl.value.trim() === '{}') argsEl.value = '{}';
}

// 对指定 MCP 做真实握手 + tools/list，并在同一窗口中提供单工具测试。
async function validateExternalMCP(name, event) {
    if (event) event.stopPropagation();
    externalMcpValidationName = name;
    openAppModal('external-mcp-validation-modal', { focus: false });
    renderExternalMCPValidationSummary(settingsT('mcp.validationLoading', '正在连接并读取工具列表…'), true);
    const toolsEl = document.getElementById('external-mcp-validation-tools');
    const panel = document.getElementById('external-mcp-tool-test-panel');
    if (toolsEl) toolsEl.innerHTML = '';
    if (panel) panel.hidden = true;
    try {
        const response = await apiFetch(`/api/external-mcp/${encodeURIComponent(name)}/validate`, { method: 'POST' });
        const data = await response.json().catch(() => ({}));
        if (!response.ok || data.ok === false) {
            throw new Error(data.error || settingsT('mcp.validationFailed', 'MCP 验证失败'));
        }
        renderExternalMCPValidationSummary(
            `${settingsT('mcp.validationSuccess', '验证成功')}：${name}\n${settingsT('mcp.toolCount', '工具数量')}：${data.tool_count || 0}\n${settingsT('mcp.validationDuration', '耗时')}：${data.duration_ms || 0} ms`,
            true
        );
        renderExternalMCPValidationTools(data.tools || []);
        await loadExternalMCPs({ forceRender: true });
        await reloadMcpToolsAfterExternalChange(true);
        if (typeof window.refreshMentionTools === 'function') window.refreshMentionTools();
    } catch (error) {
        renderExternalMCPValidationSummary(`${settingsT('mcp.validationFailed', 'MCP 验证失败')}：${error.message}`, false);
        if (toolsEl) toolsEl.innerHTML = `<div class="password-hint">${escapeHtml(settingsT('mcp.validationHint', '请根据错误检查命令、参数、环境变量、请求头以及服务是否为真正的 MCP Server。'))}</div>`;
        if (panel) panel.hidden = true;
        await loadExternalMCPs({ forceRender: true }).catch(() => {});
    }
}

async function testExternalMCPTool() {
    if (!externalMcpValidationName) return;
    const select = document.getElementById('external-mcp-test-tool');
    const argsEl = document.getElementById('external-mcp-test-args');
    const resultEl = document.getElementById('external-mcp-test-result');
    if (!select || !argsEl || !resultEl) return;
    let args;
    try {
        ...
        resultEl.textContent = `参数 JSON 无效：${error.message}`;
        return;
    }
    resultEl.hidden = false;
    resultEl.textContent = settingsT('mcp.testRunning', '正在调用工具…');
    try {
        const response = await apiFetch(`/api/external-mcp/${encodeURIComponent(externalMcpValidationName)}/test`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ tool: select.value, arguments: args }),
        });
        const data = await response.json().catch(() => ({}));
        resultEl.textContent = JSON.stringify(data, null, 2);
    } catch (error) {
        resultEl.textContent = `${settingsT('mcp.testFailed', '工具测试失败')}：${error.message}`;
    }
}

let builtinToolTestName = '';

function closeBuiltinToolTestModal() {
    if (typeof closeAppModal === 'function') {
        closeAppModal('builtin-tool-test-modal');
    } else {
        const modal = document.getElementById('builtin-tool-test-modal');
        if (modal) modal.style.display = 'none';
    }
    builtinToolTestName = '';
}

function renderBuiltinToolTestSchema(schema) {
    const summaryEl = document.getElementById('builtin-tool-test-schema-summary');
    const schemaEl = document.getElementById('builtin-tool-test-schema');
    if (!summaryEl || !schemaEl) return;

    const normalizedSchema = schema && typeof schema === 'object' ? schema : null;
    const properties = normalizedSchema && normalizedSchema.properties && typeof normalizedSchema.properties === 'object'
        ? Object.keys(normalizedSchema.properties)
        : [];
    const required = normalizedSchema && Array.isArray(normalizedSchema.required)
        ? normalizedSchema.required
        : [];
    summaryEl.textContent = properties.length > 0
        ? `${settingsT('mcp.testSchemaFields', '参数')}：${properties.join(', ')}；${settingsT('mcp.testSchemaRequired', '必填')}：${required.length ? required.join(', ') : settingsT('mcp.none', '无')}`
        : settingsT('mcp.testSchemaEmpty', '该工具未声明参数，可直接使用 {} 测试。');
    schemaEl.textContent = normalizedSchema
        ? JSON.stringify(normalizedSchema, null, 2)
        : '{}';
}

// 打开内置工具的 Schema 与真实试运行窗口。
async function openBuiltinToolTestModal(toolName, event) {
    if (event) event.stopPropagation();
    if (typeof requirePermission === 'function' && !requirePermission('mcp:execute')) return;

    builtinToolTestName = String(toolName || '').trim();
    if (!builtinToolTestName) return;

    const tool = allTools.find(item => !item.is_external && item.name === builtinToolTestName);
    const titleEl = document.getElementById('builtin-tool-test-title');
    const nameEl = document.getElementById('builtin-tool-test-name');
    const descEl = document.getElementById('builtin-tool-test-description');
    const summaryEl = document.getElementById('builtin-tool-test-schema-summary');
    const schemaEl = document.getElementById('builtin-tool-test-schema');
    const argsEl = document.getElementById('builtin-tool-test-args');
    const resultEl = document.getElementById('builtin-tool-test-result');
    if (titleEl) titleEl.textContent = `${settingsT('mcp.builtinToolTestTitle', '内置工具试运行')}：${builtinToolTestName}`;
    if (nameEl) nameEl.textContent = builtinToolTestName;
    if (descEl) descEl.textContent = tool && tool.description ? tool.description : settingsT('mcp.noDescription', '无描述');
    if (summaryEl) summaryEl.textContent = settingsT('mcp.loadingSchema', '正在加载参数 Schema…');
    ...
    openAppModal('builtin-tool-test-modal', { focus: false });

    try {
        const response = await apiFetch(`/api/config/tools/${encodeURIComponent(builtinToolTestName)}/schema`);
        const data = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(data.error || settingsT('mcp.loadSchemaFailed', '加载参数 Schema 失败'));
        renderBuiltinToolTestSchema(data.input_schema || null);
    } catch (error) {
        if (summaryEl) summaryEl.textContent = `${settingsT('mcp.loadSchemaFailed', '加载参数 Schema 失败')}：${error.message}`;
        if (schemaEl) schemaEl.textContent = '{}';
    }
}

async function testBuiltinTool() {
    if (!builtinToolTestName) return;
    if (typeof requirePermission === 'function' && !requirePermission('mcp:execute')) return;
    const argsEl = document.getElementById('builtin-tool-test-args');
    const resultEl = document.getElementById('builtin-tool-test-result');
    const button = document.getElementById('builtin-tool-test-submit');
    ...
    try {
        ...
        resultEl.textContent = `${settingsT('mcp.invalidArgumentsJson', '参数 JSON 无效')}：${error.message}`;
        return;
    }

    const warning = settingsT('mcp.builtinToolTestWarning', '这是一次真实的内置工具调用，可能执行扫描、命令或修改数据。确定继续吗？');
    ...
    try {
        const response = await apiFetch(`/api/config/tools/${encodeURIComponent(builtinToolTestName)}/test`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        resultEl.classList.toggle('is-success', !failed);
        resultEl.textContent = JSON.stringify(data, null, 2);
    } catch (error) {
        resultEl.classList.remove('is-success');
        resultEl.classList.add('is-error');
...
// 切换外部MCP启停
async function toggleExternalMCP(name, currentStatus, event) {
    if (event) event.stopPropagation();
