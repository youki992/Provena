(function () {
        ...
        while (page <= totalPages && page <= 20) {
            ...
            (data.tools || []).forEach(tool => {
                ...
                collected.push({
                    ...
                    short_description: tool.short_description || tool.shortDescription || '',
    ...
    function workflowAiDetectCapabilities(prompt) {
        return [{
            ...
            tool_name: '',
            tool_candidates: [],
            description: '根据工具用途说明、参数 schema 和实时结果动态选择工具'
        }];
    }
    ...
    function workflowAiBuildDraft(prompt, options) {
        const targetEl = document.getElementById('workflow-ai-target');
        ...
        capabilities.forEach(function (capability) {
            const id = add('agent', capability.label, {
                agent_mode: 'eino_single',
                input_binding: { from: 'previous', field: 'output' },
                instruction: '根据工具用途说明和参数 schema 自主选择、调用或停止工具，不要把场景关键词映射成固定工具。每次调用都要基于当前证据，并输出事实、证据和漏洞结果。用户需求：' + prompt,
        ...
        assumptions.push('工具由 AI 根据用途说明和实时结果动态选择，不预设场景到工具的固定映射。');
    ...
    window.useWorkflowAiExample = function (key) {
        const prompt = document.getElementById('workflow-ai-prompt');
        const target = document.getElementById('workflow-ai-target');
        if (!prompt) return;
        const exampleKey = 'workflows.ai.examples.' + key;
        prompt.value = _t(exampleKey);
        if (key === 'pentestLoop' && target && !target.value.trim()) {
