...
var DASHBOARD_SEVERITY_STATUS_FILTER_STORAGE_KEY = 'provena.dashboard.severityStatusFilter';
...
// 仪表盘的「查看全部」使用 JS 路由，不依赖真实 href；补齐键盘入口，
// 保证鼠标点击和 Enter/Space 联动到同一目标。
function initDashboardKeyboardNavigation() {
    var page = document.getElementById('page-dashboard');
    if (!page) return;
    page.querySelectorAll('.dashboard-section-link').forEach(function (link) {
        if (link.dataset.dashboardKeyboardReady === 'true') return;
        link.dataset.dashboardKeyboardReady = 'true';
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initDashboardKeyboardNavigation);
} else {
    initDashboardKeyboardNavigation();
}
