...
function resolveWebshellAiStreamRequest() {
    ...
    return apiFetch('/api/config').then(function (r) {
        ...
        if (typeof window.csaiChatAgentMode === 'object' && typeof window.csaiChatAgentMode.normalizeStored === 'function') {
            norm = window.csaiChatAgentMode.normalizeStored(localStorage.getItem('provena-chat-agent-mode'), cfg);
        } else {
            var mode = localStorage.getItem('provena-chat-agent-mode');
...
function wsInitAgentMode() {
    ...
    apiFetch('/api/config').then(function (r) { return r.ok ? r.json() : null; }).then(function (cfg) {
        ...
        // 标准化当前值
        var stored = localStorage.getItem('provena-chat-agent-mode');
        var norm;
        if (typeof window.csaiChatAgentMode === 'object' && typeof window.csaiChatAgentMode.normalizeStored === 'function') {
...
function wsSelectAgentMode(mode) {
    try { localStorage.setItem('provena-chat-agent-mode', mode); } catch (e) { /* */ }
...
function wsRefreshSelectors() {
    ...
    wsRenderRoleList();
    wsUpdateProjectButtonLabel();
    var stored = localStorage.getItem('provena-chat-agent-mode') || 'eino_single';
...
function buildWebshellTimelineItemFromDetail(detail) {
    ...
    if (eventType === 'iteration') {
        ...
        var wsCallTitle = typeof window.formatToolCallTimelineTitle === 'function'
            ...
            : ((typeof window.t === 'function') ? (total > 0 ? window.t('chat.callTool', { name: tn, index: idx, total: total }) : window.t('chat.callToolNoProgress', { name: tn })) : ('调用: ' + tn + (total ? ' (' + idx + '/' + total + ')' : '')));
                ...
                try {
                    ...
                    // ─── Tool calls ───
                        ...
                        var callTitle = typeof window.formatToolCallTimelineTitle === 'function'
                            ? window.formatToolCallTimelineTitle(tn, idx, total)
                            : (total > 0 ? (wsTOr('chat.callTool', '') || ('调用工具: ' + tn + ' (' + idx + '/' + total + ')')) : (wsTOr('chat.callToolNoProgress', '') || ('调用工具: ' + tn)));
