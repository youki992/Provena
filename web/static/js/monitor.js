...
function ensureStreamTimeline(progressId) {
    const existing = resolveStreamTimeline(progressId);
    if (existing) return existing;
    const progress = progressId ? document.getElementById(progressId) : null;
    const container = progress && progress.querySelector('.progress-container');
    if (!container) return null;
    const timeline = document.createElement('div');
    timeline.id = String(progressId) + '-timeline';
    timeline.className = 'progress-timeline expanded';
...
function handleStreamEvent(event, progressElement, progressId, 
    ...
    if (expectedConversationId) {
        ...
        if (
            ...
            !(options && options.allowProgressWhenConversationDiff && progressNode)
        ) {
            return;
        }
    }
    ...
    const timeline = resolveStreamTimeline(progressId) || (!canHandleWithoutTimeline ? ensureStreamTimeline(progressId) : null);
    ...
    switch (event.type) {
        ...
        case 'reasoning_chain_stream_start': {
            ...
            const itemId = addTimelineItem(timeline, timelineType, {
                title: title,
                message: event.message || 'Pi 已开始分析，等待思考摘要…',
                data: d
            });
        ...
        case 'tool_call':
            ...
            // Pi executes one bridge call per RPC round. Older persisted Pi
            // events may not have carried the progress fields, so keep them
            // readable as 1/1 instead of showing the meaningless 0/0.
            const isPiTool = String(toolInfo.source || '').toLowerCase() === 'pi';
            const index = Number(toolInfo.index) > 0 ? Number(toolInfo.index) : (isPiTool ? 1 : 0);
            const total = Number(toolInfo.total) > 0 ? Number(toolInfo.total) : (isPiTool ? 1 : 0);
...
function formatToolCallTimelineTitle(toolName, index, total) {
    ...
    if (tot <= 0) {
        return typeof window.t === 'function'
            ? window.t('chat.callToolNoProgress', { name: name })
            : '调用工具: ' + name;
    }
    if (typeof window.t === 'function') {
...
function addTimelineItem(timeline, type, options) {
    ...
    if (type === 'tool_call' && options.data) {
        ...
        const isPiTool = String(d.source || '').toLowerCase() === 'pi';
        item.dataset.toolSource = String(d.source || '');
        item.dataset.toolIndex = Number(d.index) > 0 ? String(d.index) : (isPiTool ? '1' : '0');
        item.dataset.toolTotal = Number(d.total) > 0 ? String(d.total) : (isPiTool ? '1' : '0');
...
function refreshProgressAndTimelineI18n() {
    ...
    document.querySelectorAll('.timeline-item').forEach(function (item) {
        ...
        if (type === 'iteration' && item.dataset.iterationN) {
            ...
            titleSpan.textContent = ap + '\u2699\uFE0F ' + (typeof window.t === 'function' ? window.t('chat.executing') : '执行中');
            ...
            const isPiTool = String(item.dataset.toolSource || '').toLowerCase() === 'pi';
            const index = parseInt(item.dataset.toolIndex, 10) || (isPiTool ? 1 : 0);
            const total = parseInt(item.dataset.toolTotal, 10) || (isPiTool ? 1 : 0);
            const callTitle = typeof formatToolCallTimelineTitle === 'function'
                ? formatToolCallTimelineTitle(name, index, total)
                : (total > 0 ? _t('chat.callTool', { name: name, index: index, total: total }) : _t('chat.callToolNoProgress', { name: name }));
