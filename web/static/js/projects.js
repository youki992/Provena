...
const PROJECTS_LIST_PAGE_SIZE_KEY = 'provena.projects_list_page_size';
let currentProjectId = null;
let currentProjectUpdatedAt = null;
let currentProjectTab = 'facts';
let currentProjectAssets = [];
const PROJECT_ASSETS_PAGE_SIZE_KEY = 'provena.project_assets_page_size';
...
const PROJECT_ACTIVE_KEY = 'provena.activeProjectId';
...
async function initProjectsPage() {
    ...
    const hashProjectId = (hashParams.get('id') || '').trim();
    const hashFactKey = (hashParams.get('fact_key') || '').trim();
    if (hashProjectId && projectsCache.some((item) => item.id === hashProjectId)) {
        currentProjectId = hashProjectId;
    } else if (!currentProjectId && projectsCache.length) {
        currentProjectId = projectsCache[0].id;
    }
    renderProjectsSidebar();
    if (currentProjectId) {
        await selectProject(currentProjectId);
        if (hashFactKey && currentProjectId === hashProjectId) {
            switchProjectTab('facts');
            await viewProjectFactBody(hashFactKey);
        }
    }
}
...
async function loadProjectsList() {
    ...
    try {
        ...
        // A database switch or restore can invalidate the project id retained
        // by the current page/localStorage. Clear it before any tab loader
        // issues /fact-graph requests, otherwise the UI keeps asking for a
        // project that only existed in the previous database.
        const availableProjectIDs = new Set(projectsCacheAll.map((item) => item && item.id).filter(Boolean));
        if (currentProjectId && !availableProjectIDs.has(currentProjectId)) {
        ...
        if (storedProjectId && !availableProjectIDs.has(storedProjectId)) {
            setActiveProjectId('');
        }
    } catch (e) {
        console.warn(e);
...
async function selectProject(id) {
    ...
    try {
        ...
        if (!res.ok) {
            if (res.status === 404 && currentProjectId === id) {
                currentProjectId = null;
                currentProjectUpdatedAt = null;
                setActiveProjectId('');
                updateProjectsDetailVisibility();
            }
            throw new Error(tp('projects.projectNotFound'));
        }
