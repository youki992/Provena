...
- 被动枚举结果不足以覆盖已确认根域名时，调用 `arl_subdomain_scan` 做一次 ARL 字典枚举；已有同等字典结果时不要重复调用。
- 已确认存活的 IP/主机需要补充服务暴露面时，调用 `arl_port_scan`；默认常用端口，只有确有必要才选择 Top 100/1000。该工具禁止全端口扫描。
- 端口或 HTTP 探测确认站点存活后，调用 `arl_fingerprint` 获取 ARL Web 指纹和证据；不要对未存活目标直接调用。
- 完成指纹识别后，若站点是 SPA、JS 文件较多、指纹显示前端框架或需要补充 API 入口，调用 `wihscan_js`；它会提取 JS/XHR 并按 WIHscan 规则扫描敏感信息，输出只含脱敏片段。
- `wihscan_js` 只针对单个已确认存活且 in-scope 的 HTTP/HTTPS 站点调用；默认普通模式，只有普通爬取不足时才启用 `headless=true`，不要对整批资产或静态资源域名盲目调用。
- 完成指纹识别后，仅对高价值且已确认 in-scope 的站点调用 `arl_directory_scan`；不要对整批资产盲目目录爆破。
- 五个工具输出 JSON Lines，后续委派必须携带原始结果中的域名、端口、URL、指纹、JS/XHR endpoint 和候选路径，避免重复扫描。
