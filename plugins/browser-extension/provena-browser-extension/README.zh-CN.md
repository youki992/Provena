...
Chrome / Edge（Chromium）DevTools 扩展：在开发者工具中捕获 **Network** 流量，发送到 Provena 进行 AI 辅助安全测试。能力与 Burp Suite 插件对齐，并按生产场景做了性能与体验优化。
...
2. 选择目录：`plugins/browser-extension/provena-browser-extension/`
3. 打开目标页面 → **F12** → 顶部 Tab **Provena**
...
- 权限申请直接绑定 Validate 点击事件，仅申请当前 Provena 服务 origin；已授权地址不会重复弹窗
...
升级并重启 Provena 服务。新版服务会自动识别格式合法的 Chrome/Edge 扩展 Origin，无需复制插件 ID 或配置 CORS 白名单；插件首次 Validate 时仍会请求访问目标服务地址的浏览器权限。

**Validate 要求允许访问 Provena 服务？**  
...
```bash
bash plugins/browser-extension/provena-browser-extension/package.sh
# → dist/provena-browser-extension.zip
```
...
ICONS="plugins/browser-extension/provena-browser-extension/icons"
