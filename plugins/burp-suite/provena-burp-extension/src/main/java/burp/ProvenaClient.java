...
final class ProvenaClient {
    ...
    String loginAndValidate(Config cfg) throws IOException {
        ...
        java.util.Timer deadline = new java.util.Timer("Provena-AuthDeadline", true);
    ...
    private String login(String baseUrl, String password) throws IOException {
        ...
        if (looksLikeHtml(resp) || (contentType != null && contentType.toLowerCase().contains("text/html"))) {
            throw new IOException("Login failed: server returned HTML, not API JSON. Check IP/Port and ensure you point to Provena backend.");
        }
