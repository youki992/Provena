## Provena Burp Suite 插件（中文说明）

### 功能概述

- 在 Burp 的 `Provena` 标签页中配置 **Host、端口、密码**
- 点击 **Validate（验证）**：
  - 调用 `POST /api/auth/login` 用密码换取 Token
  - 调用 `GET /api/auth/validate` 校验 Token
  - 验证通过后 Token 会保存在插件内存中（本次 Burp 会话有效）
- 右键任意 HTTP 请求包 → **Send to Provena (stream test)**：
  - 弹出发送对话框，可针对**当前流量**选择：
...
```bash
cd plugins/burp-suite/provena-burp-extension
./build-mvn.sh
```

产物：

- `dist/provena-burp-extension.jar`
...
- **右键发送后无流式输出**
  ...
  - 确认 Burp 能访问到 Provena（网络/代理/防火墙）
